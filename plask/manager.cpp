...
Manager::SetAxisNames::SetAxisNames(Manager& manager, const AxisNames* names) : manager(manager), old(manager.axisNames) {
    manager.axisNames = names;
}

Manager::SetAxisNames::SetAxisNames(Manager& manager, XMLReader& source) : manager(manager), old(manager.axisNames) {
...
bool Manager::tryLoadFromExternal(XMLReader& reader, const Manager::LoadFunCallbackT& load_from) {
    ...
    load(new_loader.first, materialsDB, *new_loader.second, [&](const std::string& section_name) -> bool { return section_name ==
    section_to_load; }); return true;*/
}
...
void Manager::loadFromReader(XMLReader& reader, const LoadFunCallbackT& load_from_cb) { load(reader, load_from_cb); }
...
void Manager::loadGeometry(GeometryReader& greader) {
    ...
    while (greader.source.requireTagOrEnd()) roots.push_back(greader.readGeometry());
}
...
void Manager::loadMaterial(XMLReader& reader) {
    writelog(LOG_ERROR, "Loading XML material from C++ not implemented (ignoring material {})",
             reader.getAttribute<std::string>("name", "unknown"));
    reader.gotoEndOfCurrentTag();
}
...
void Manager::loadGrids(XMLReader& reader) {
    ...
    while (reader.requireTagOrEnd()) {
        if (reader.getNodeName() == "mesh") {
            ...
            if (meshes.find(name) != meshes.end()) throw NamesConflictException("mesh or mesh generator", name);
            ...
            if (reader.getNodeType() != XMLReader::NODE_ELEMENT_END || reader.getNodeName() != "generator")
                throw Exception(
                    "internal error in {0} (method: {1}) mesh generator reader, after return reader does not point to end of "
                    "generator tag.",
                    type, method);
...
shared_ptr<Solver> Manager::loadSolver(const std::string&, const std::string&, const std::string&, const std::string& name) {
    ...
    if (found == solvers.end())
        throw Exception(
            "in C++ solvers ('{0}' in this case) must be created and added to Manager::solvers manually before reading XML.", name);
    solvers.erase(
        found);  // this is necessary so we don't have name conflicts — the solver will be added back to the map by loadSolvers
    return found->second;
}
...
void Manager::loadSolvers(XMLReader& reader) {
    ...
    while (reader.requireTagOrEnd()) {
        ...
        if (shared_ptr<Solver> filter = FiltersFactory::getDefault().get(reader, *this)) {
            if (!this->solvers.insert(std::make_pair(name, filter)).second) throw NamesConflictException("solver", name);
            continue;
        }
        ...
        if (!this->solvers.insert(std::make_pair(name, solver)).second) throw NamesConflictException("solver", name);
    }
...
Notatki:
1. obiekt istotny w geometrii to obiekt mający nazwę, taki że na ścieżce od korzenia (geometrii) do tego obiektu nie ma innego
obiektu mającego nazwe mozna je znaleźć przechodząc drzew w głąb aż do napotkania obiektu mającego nazwę (wtedy przestajemy schodzić
w głąb)
...
struct PositionValidator {
    ...
    void fill(const GeometryObject* obj, Set& s) {
        ...
        if (geom_to_name.count(obj))
            ...
            for (std::size_t i = 0; i < c; ++i) fill(obj->getRealChildNo(i).get(), s);
        }
    }
    ...
    template <typename VectorType> bool compare_vec(std::vector<VectorType> v1, std::vector<VectorType> v2) {
        if (v1.empty() || v2.empty()) return true;
        std::sort(v1.begin(), v1.end());
        std::sort(v2.begin(), v2.end());
        if (v1.size() == v2.size()) {
            return std::equal(v1.begin(), v1.end(), v2.begin(), [](const VectorType& x1, const VectorType& x2) {
                return isnan(x1) || isnan(x2) || x1.equals(x2, 1.1e-4);
            });
        }  // different sizes:
     ...
     */
    template <typename GeomType> Set compare_d(const GeomType* geom1, const GeomType* geom2) {
...
void Manager::validatePositions(const std::function<void(const Geometry*,
                                                         const Geometry*,
                                                         std::vector<const GeometryObject*>&&,
                                                         const std::map<const GeometryObject*, const char*>&)>& callback) const {
    ...
    for (auto& geom : roots) {
        auto ptr = geom.get();
        geometries_by_type[std::type_index(typeid(*ptr))].insert(ptr);
    }
    if (std::find_if(geometries_by_type.begin(), geometries_by_type.end(),
                     [](GeomToType::value_type& v) { return v.second.size() > 1; }) == geometries_by_type.end())
        return;  // no 2 geometries of the same type

    PositionValidator::GeomToName geom_to_name;
    for (auto& i : geometrics) geom_to_name[i.second.get()] = i.first.c_str();
    ...
    for (auto& geom_set : geometries_by_type)
        for (auto it = geom_set.second.begin(); it != geom_set.second.end(); ++it) {
            ...
            for (; it2 != geom_set.second.end(); ++it2) {
                ...
                if (!objs.empty()) callback(*it, *it2, std::move(objs), geom_to_name);
            }
        }
}
...
void Manager::validatePositions() const {
    ...
    validatePositions([this](const Geometry* g1, const Geometry* g2, std::vector<const GeometryObject*>&& objs,
                             const std::map<const GeometryObject*, const char*>& names) {
        bool single = objs.size() < 2;
        std::string ons;
        for (auto o : objs) {
            ons += " '";
            ons += names.find(o)->second;
            ons += '\'';
        }
        writelog(plask::LOG_WARNING, "Object{}{} ha{} different position in geometry {} and {}", single ? "" : "s", ons,
                 single ? "s" : "ve", geomName(*this, g1, names), geomName(*this, g2, names));
    });
}
