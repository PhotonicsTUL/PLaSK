...
template <int dim> const char* ArrangeContainer<dim>::NAME = dim == 2 ? PLASK_ARRANGE2D_NAME : PLASK_ARRANGE3D_NAME;
...
template <int dim> void ArrangeContainer<dim>::warmOverlaping() const {
    if (warn_overlapping && this->hasChild()) {
        Box box = this->_child->getBoundingBox();
        box -= box.lower;
        if (box.intersects(box + translation)) writelog(LOG_WARNING, "Arrange: item bboxes overlap");
    }
}

template <int dim> typename ArrangeContainer<dim>::Box ArrangeContainer<dim>::getBoundingBox() const {
    Box bbox;
    if (!this->hasChild()) {
        bbox.makeInvalid();
    } else {
...
template <int dim> typename ArrangeContainer<dim>::Box ArrangeContainer<dim>::getRealBoundingBox() const {
...
void ArrangeContainer<dim>::getBoundingBoxesToVec(const GeometryObject::Predicate& predicate,
                                                  std::vector<typename ArrangeContainer<dim>::Box>& dest,
    ...
    for (unsigned r = 1; r < repeat_count; ++r)
        for (std::size_t i = old_size; i < new_size; ++i) dest.push_back(dest[i] + translation * r);
}
...
void ArrangeContainer<dim>::getPositionsToVec(const GeometryObject::Predicate& predicate,
                                              std::vector<typename ArrangeContainer<dim>::DVec>& dest,
                                              const PathHints* path) const {
    if (predicate(*this)) {
...
template <int dim> bool ArrangeContainer<dim>::contains(const typename ArrangeContainer<dim>::DVec& p) const {
...
template <int dim> std::size_t ArrangeContainer<dim>::getChildrenCount() const {
    return this->hasChild() ? repeat_count : 0;
}

template <int dim> shared_ptr<GeometryObject> ArrangeContainer<dim>::getChildNo(std::size_t child_no) const {
...
template <int dim> std::size_t ArrangeContainer<dim>::getRealChildrenCount() const {
    return GeometryObjectTransform<dim>::getChildrenCount();
}

template <int dim> shared_ptr<GeometryObject> ArrangeContainer<dim>::getRealChildNo(std::size_t child_no) const {
    return GeometryObjectTransform<dim>::getRealChildNo(child_no);
}

template <int dim>
GeometryObject::Subtree ArrangeContainer<dim>::getPathsAt(const typename ArrangeContainer<dim>::DVec& point,
                                                          bool all) const {
...
template <int dim> shared_ptr<GeometryObject> ArrangeContainer<dim>::shallowCopy() const {
    return plask::make_shared<ArrangeContainer<dim>>(_child, translation, repeat_count);
}

template <int dim>
typename ArrangeContainer<dim>::Box ArrangeContainer<dim>::fromChildCoords(
    const typename ArrangeContainer<dim>::ChildType::Box& child_bbox) const {
    return child_bbox;
}
...
template <int dim>
void ArrangeContainer<dim>::addLineSegmentsToSet(std::set<typename GeometryObjectD<dim>::LineSegment>& segments,
                                                 unsigned max_steps,
                                                 double min_step_size) const {
    if (!this->hasChild()) return;
    std::set<typename GeometryObjectD<dim>::LineSegment> child_segments;
    _child->addLineSegmentsToSet(child_segments, this->max_steps ? this->max_steps : max_steps,
                                 this->min_step_size ? this->min_step_size : min_step_size);
    for (int i = 0; i < repeat_count; ++i) {
        DVec trans = i * translation;
        for (const auto& p : child_segments)
            segments.insert(typename GeometryObjectD<dim>::LineSegment(p[0] + trans, p[1] + trans));
    }
}
...
void Lattice::getBoundingBoxesToVec(const GeometryObject::Predicate& predicate,
                                    std::vector<Box>& dest,
                                    const PathHints* path) const {
    if (predicate(*this)) {
        dest.push_back(getBoundingBox());
    ...
    container->forEachChild([&](const Translation<3>& child) { child.getBoundingBoxesToVec(predicate, dest, path); },
                            path);
}

void Lattice::getObjectsToVec(const GeometryObject::Predicate& predicate,
                              std::vector<shared_ptr<const GeometryObject>>& dest,
                              const PathHints* path) const {
    if (predicate(*this)) {
        dest.push_back(this->shared_from_this());
...
void Lattice::getPositionsToVec(const GeometryObject::Predicate& predicate,
...
void Lattice::refillContainer() {
    ...
    std::map<int, std::set<int>>
        result;  // coordinates of all (x, y) points inside the polygon in the map: y -> set of x
    YEnds ends;
    SegmentsIterator segment(this->segments);
    while (segment.next()) {
        if (segment.first.c1 == segment.second.c1) {
            std::set<int>& dst = result[segment.first.c1];
            if (segment.first.c0 > segment.second.c0) std::swap(segment.first.c0, segment.second.c0);
...
void Lattice::addLineSegmentsToSet(std::set<typename GeometryObjectD<3>::LineSegment>& segments,
                                   ...
                                   double min_step_size) const {
    if (!this->hasChild()) return;
    container->addLineSegmentsToSet(segments, this->max_steps ? this->max_steps : max_steps,
                                    this->min_step_size ? this->min_step_size : min_step_size);
}
...
void Lattice::writeXMLChildren(XMLWriter::Element& dest_xml_object,
...
shared_ptr<GeometryObject> read_lattice(GeometryReader& reader) {
    ...
    for (const std::string& t : tokens) {
        if (t == ";" || t == "^") {  // end of point or segment
            ...
            if (cords_in_current_point == 2)
                throw Exception("End of point (\";\") or segment (\"^\") was expected, but got \"{0}\".", t);
