...
static void old__interpolate__to__remove(const RectangularMesh<2>& src_mesh, const DataVector<const SrcT>& src_vec,
    ...
    if (n0 > 1 && n1 > 1) {
        ...
        for (int di = 0; di < dst_mesh.size(); ++di) {
            ...
            for (int j0 = max(i0-1,0); j0 < min(i0+1,n0); ++j0) {
                for (int j1 = max(i1-1,0); j1 < min(i1+1,n1); ++j1) {
                    ...
                    if (!have_diff[j]) { // we need to compute derivatives
                        ...
                        const double dl = src_mesh.axis0->at(j0) - src_mesh.axis0->at(j0l),
                                     dr = src_mesh.axis0->at(j0r) - src_mesh.axis0->at(j0),
                                     db = src_mesh.axis1->at(j1) - src_mesh.axis0->at(j1b),
                                     dt = src_mesh.axis1->at(j1t) - src_mesh.axis0->at(j1);
            ...
            double d0 = src_mesh.axis0->at(i0) - src_mesh.axis0->at(i0-1),
                   d1 = src_mesh.axis1->at(i1) - src_mesh.axis1->at(i1-1);
            double x0 = (p.c0 - src_mesh.axis0->at(i0-1)) / d0,
                   x1 = (p.c1 - src_mesh.axis1->at(i1-1)) / d1;
...
struct SplineRect2DLazyDataImpl: public InterpolatedLazyDataImpl<DstT, RectangularMesh<2>, SrcT > {

    DataVector<DstT> result;   //TODO remove this line

    SplineRect2DLazyDataImpl(const shared_ptr<const RectangularMesh<2>>& src_mesh, const DataVector<const SrcT>& src_vec, const shared_ptr<const MeshD<2>>& dst_mesh)
        : InterpolatedLazyDataImpl<DstT, RectangularMesh<2>, SrcT>(src_mesh, src_vec, dst_mesh)
            , result(dst_mesh->size())     //TODO remove this line
    {
        old__interpolate__to__remove(*src_mesh, src_vec, *dst_mesh, result);  //TODO remove this line
    }

    virtual DstT at(std::size_t index) const override {
        //this->src_mesh, this->src_vec, this->dst_mesh are available here
        return result[index];   //TODO remove this line
    }
...
struct InterpolationAlgorithm<RectangularMesh<2>, SrcT, DstT, INTERPOLATION_SPLINE> {
    static LazyData<DstT> interpolate(const shared_ptr<const RectangularMesh<2>>& src_mesh, const DataVector<const SrcT>& src_vec, const shared_ptr<const MeshD<2>>& dst_mesh) {
        ...
        return new SplineRect2DLazyDataImpl<DstT, SrcT>(src_mesh, src_vec, dst_mesh);
    }
