...
MATERIALS_PROPERTES = {
    'A': (u'Monomolecular recombination coefficient <i>A</i>', u'1/s', [(u'T', u'temperature', u'K')]),
    'absp':
    (u'Absorption coefficient <i>α</i>', u'cm<sup>-1</sup>', [(u'lam', u'wavelength', u'nm'), (u'T', u'temperature', u'K')]),
    'ac': (
        u'Hydrostatic deformation potential for the conduction band <i>a</i><sub><i>c</i></sub>', u'eV',
        [(u'T', u'temperature', u'K')]
    ),
    'av':
    (u'Hydrostatic deformation potential for the valence band <i>a</i><sub><i>v</i></sub>', u'eV', [(u'T', u'temperature', u'K')]),
    'B': (u'Radiative recombination coefficient <i>B</i>', u'cm<sup>3</sup>/s', [(u'T', u'temperature', u'K')]),
    'b': (u'Shear deformation potential <i>b</i>', u'eV', [(u'T', u'temperature', u'K')]),
    'C': (u'Auger recombination coefficient <i>C</i>', u'cm<sup>6</sup>/s', [(u'T', u'temperature', u'K')]),
    'Ce': (
        u'Auger recombination coefficient <i>C</i><sub><i>e</i></sub> for electrons', u'cm<sup>6</sup>/s',
        [(u'T', u'temperature', u'K')]
    ),
    'Ch':
    (u'Auger recombination coefficient <i>C</i><sub><i>h</i></sub> for holes', u'cm<sup>6</sup>/s', [(u'T', u'temperature', u'K')]),
    'c11': (u'Elastic constant <i>c<sub>11</sub></i>', u'GPa', [(u'T', u'temperature', u'K')]),
    'c12': (u'Elastic constant <i>c<sub>12</sub></i>', u'GPa', [(u'T', u'temperature', u'K')]),
    'c13': (u'Elastic constant <i>c<sub>13</sub></i>', u'GPa', [(u'T', u'temperature', u'K')]),
    'c33': (u'Elastic constant <i>c<sub>33</sub></i>', u'GPa', [(u'T', u'temperature', u'K')]),
    'c44': (u'Elastic constant <i>c<sub>44</sub></i>', u'GPa', [(u'T', u'temperature', u'K')]),
    'CB': (
        u'Conduction band level <i>CB</i>', u'eV', [(u'T', u'temperature', u'K'), (u'e', u'lateral strain', '-'),
                                                    (u'point', u'point in the Brillouin zone', '-')]
    ),
    'chi': (
        u'Electron affinity <i>χ</i>', u'eV', [(u'T', u'temperature', u'K'), (u'e', u'lateral strain', '-'),
                                               (u'point', u'point in the Brillouin zone', '-')]
    ),
    'cond': (u'Electrical conductivity <i>σ</i> in lateral and vertical directions', u'S/m', [(u'T', u'temperature', u'K')]),
    'condtype':
    (u'Electrical conductivity type. '
     u'In semiconductors this indicates what type of carriers <i>Nf</i> refers to.', u'-', []),
    'cp': (u'Specific heat at constant pressure', u'J/(kg K)', [(u'T', u'temperature', u'K')]),
    ...
    'mobe': (
        u'Electron mobility <i>µ</i><sub><i>e</i></sub> in lateral and vertical directions', u'cm<sup>2</sup>/Vs',
        [(u'T', u'temperature', u'K')]
    ),
    'mobh': (
        u'Hole mobility <i>µ</i><sub>name_re.sub("", <i>h</i></sub> in lateral and vertical directions', u'cm<sup>2</sup>/Vs',
        [(u'T', u'temperature', u'K')]
    ),
    'Mso': (
        u'Split-off mass <i>M</i><sub>so</sub>', u'<i>m</i><sub>0</sub>', [(u'T', u'temperature', u'K'),
    ...
    'Nf': (u'Free carrier concentration <i>N</i>', u'cm<sup>-3</sup>', [(u'T', u'temperature', u'K')]),
    'Ni': (u'Intrinsic carrier concentration <i>N</i><sub><i>i</i></sub>', u'cm<sup>-3</sup>', [(u'T', u'temperature', u'K')]),
    'Nr': (
        u'Complex refractive index <i>n</i><sub><i>R</i></sub>',
        u'-', [(u'lam', u'wavelength', u'nm'), (u'T', u'temperature', u'K'),
               (u'n', u'injected carriers concentration', 'cm<sup>-1</sup>')]
    ),
    'nr': (
    ...
    'tauh': (u'Monomolecular holes lifetime <i>A</i><sub><i>h</i></sub>', u'ns', [(u'T', u'temperature', u'K')]),
    'thermk': (
        u'Thermal conductivity in lateral and vertical directions <i>k</i>', u'W/(m K)', [(u'T', u'temperature', u'K'),
                                                                                          (u'h', u'layer thickness', u'µm')]
    ),
    'VB': (
        u'Valance band level offset <i>VB</i>', u'eV', [(u'T', u'temperature', u'K'), (u'e', u'lateral strain', '-'),
                                                        (u'point', u'point in the Brillouin zone', '-'),
...
def extract_material_name(material):
    return name_re.sub("", material)
                    ...
                    with OrderedTagReader(mat) as props:
                        with AttributeReader(mat) as mat_attrib:
                            ...
                            material = MaterialsModel.Material(
                                self, mat_attrib.get('name', ''), base, properties, alloy, mat.comments, props.get_comments()
                            )
    ...
    def data(self, index, role=Qt.ItemDataRole.DisplayRole):
        if isinstance(self.entries[index.row()], MaterialsModel.External):
            if index.column() == 1:
                ...
                return QIcon.fromTheme({'library': 'material-library', 'module': 'material-module'}[self.entries[index.row()].what])
    ...
    def create_info(self):
            ...
            if isinstance(d, MaterialsModel.Material):
                ...
                if d.alloy:
                    ...
                    if len(''.join(elements)) != len(name):
                        res.append(
                            Info(
                                u"Alloy material's name does not consist of elements "
                                u"and optional label [row: {}]".format(i + 1),
                                Info.ERROR,
                                rows=[i],
                                cols=[0]
                            )
                    ...
                    try:
                        ...
                        if not (
                            ...
                            if extract_material_name(mat) not in self.get_materials(i):
                                res.append(
                                    Info(
                                        u"Material base '{1}' is not a proper material ({2}) [row: {0}]".format(i + 1, d.base, err),
                                        Info.ERROR,
                                        rows=[i],
                                        cols=[1]
                                    )
        ...
        for name, rows in names.items():
            if len(rows) > 1:
                res.append(
                    Info(
                        u'Duplicated material name "{}" [rows: {}]'.format(name, ', '.join(str(i + 1) for i in rows)),
                        Info.ERROR,
                        rows=rows,
                        cols=[0]
                    )
        ...
        with HandleMaterialsModule(self.document, partial(self.RegisterWrap, container=container)):
            for material in entries:
                if isinstance(material, MaterialsModel.External):
                    ...
                    try:
                        model_materials.remove(material.name)
                    except ValueError:
                        pass
