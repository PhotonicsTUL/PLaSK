...
class Grid(InfoSource): # or (TreeFragmentModel)??
    ...
    def set_text(self, text):
        if self.is_generator:
            tab = ['<generator name=', quoteattr(self.name).encode('utf-8'), ' type=', quoteattr(self.type).encode('utf-8'), ' method=', quoteattr(self.method).encode('utf-8'), '>', text.encode('utf-8'), '</generator>' ]
        else:
            tab = ['<mesh name=', quoteattr(self.name).encode('utf-8'), ' type=', quoteattr(self.type).encode('utf-8'), '>', text.encode('utf-8'), '</mesh>' ]
        #print ''.join(tab)
