...
class VerticalScrollArea(QScrollArea):
    ...
    def eventFilter(self, obj, event):
        if isinstance(obj, QWidgetItem): obj = obj.widget()  # hacky fix
        if obj and obj == self.widget() and isinstance(event, QEvent) and event.type() == QEvent.Type.Resize:
...
class MultiLineEdit(QWidget):
    ...
    def __init__(self, parent=None, movable=False, change_cb=None, document=None, compact=True):
        ...
        if movable:
            ...
            act = QAction(self.list_widget)
            act.setShortcut(QKeySequence(Qt.Modifier.CTRL | Qt.Modifier.SHIFT | Qt.Key.Key_Up))
            ...
            act = QAction(self.list_widget)
            act.setShortcut(QKeySequence(Qt.Modifier.CTRL | Qt.Modifier.SHIFT | Qt.Key.Key_Down))
