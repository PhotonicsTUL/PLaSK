...
void ObjectWrapper::drawMiniature(QPainter& painter, qreal w, qreal h, bool saveProp) const {
    plask::GeometryObject& toDraw = *wrappedObject;
    ...
    if (saveProp) {
        ...
        painter.scale(w / s.tran(), h / s.vert());
...
QPixmap ObjectWrapper::getMiniature(qreal w, qreal h, bool saveProp) const {
    ...
    drawMiniature(painter, w/*-1.0*/, h/*-1.0*/, saveProp);   //-1.0 for typical pen size, if borders are draw
    return result;
}
