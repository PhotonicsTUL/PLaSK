...
void register_geometry_primitive()
{
    ...
    py::class_<Box2D, shared_ptr<Box2D>>("Box2D",
        ...
        .def(py::self == py::self)
        .def(py::self != py::self)
        .def("includes", &Box2D::includes, py::args("point"), "Check if the point is inside the box.")
    ...
    py::class_<Box3D, shared_ptr<Box3D>>("Box3D",
        ...
        .def(py::self == py::self)
        .def(py::self != py::self)
        .def("includes", &Box3D::includes, "Check if the point is inside the box.")
        .def("intersects", &Box3D::intersects, "Check if this and the other box have common points.")
        .def("extend", includeR3p, (py::arg("point")), "Make this box the minimal one which includes this and the given point")
        .def("extend", includeR3R, (py::arg("other")), "Make this box the minimal one which includes this and the other box.")
        .def("translated", &Box2D::translated, py::args("trans"), "Get translated copy of this box")
        .def("translate", &Box2D::translate, py::args("trans"), "Translate this box")
