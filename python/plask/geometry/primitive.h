...
static shared_ptr<Rect2d> Rect2d_constructor_2vec(const Vec2<double>& lower, const Vec2<double>& upper) {
...
static shared_ptr<Rect2d> Rect2d_constructor_4numbers(double x1, double y1, double x2, double y2) {
    shared_ptr<Rect2d> R { new Rect2d(Vec2<double>(x1,y1), Vec2<double>(x2,y2)) };
    R->fix();
    return R;
}
// __str__(v)
...
// Init constructor wrap
static shared_ptr<Rect3d> Rect3d_constructor_2vec(const Vec3<double>& lower, const Vec3<double>& upper) {
...
static shared_ptr<Rect3d> Rect3d_constructor_4numbers(double x1, double y1, double z1, double x2, double y2, double z2) {
    shared_ptr<Rect3d> R { new Rect3d(Vec3<double>(x1,y1,z1), Vec3<double>(x2,y2,z2)) };
    R->fix();
    return R;
}
// __str__(v)
...
{
    void (Rect2d::*includeR2p)(const Vec2<double>&) = &Rect2d::include;
    ...
    py::class_<Rect2d,shared_ptr<Rect2d>>("Rect2D", "Rectangle class. Allows for some basic operation on rectangles.")
        ...
        .def("fix", &Rect2d::fix, "Ensure that lower.x <= upper.x and lower.y <= upper.y. Change x or y of lower and upper if necessary.")
        .def(py::self == py::self)
        .def(py::self != py::self)
    ...
    void (Rect3d::*includeR3p)(const Vec3<double>&) = &Rect3d::include;
    ...
    py::class_<Rect3d,shared_ptr<Rect3d>>("Rect3D", "Cuboid class. Allows for some basic operation on rectangles.")
        ...
        .def("fix", &Rect3d::fix, "Ensure that lower.x <= upper.x, lower.y <= upper.y, and lower.z <= upper.z Change x, y, or z of lower and upper if necessary.")
        .def(py::self == py::self)
        .def(py::self != py::self)
