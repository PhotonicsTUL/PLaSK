...
void PythonManager::makeWeakRef(const py::object& self) { globals["self"] = py::handle<>(PyWeakref_NewProxy(self.ptr(), nullptr)); }
...
PLASK_PYTHON_API void loadXpl(py::object self, py::object src, py::dict vars, py::object filter = py::object()) {
    ...
    if (vars.has_key("self")) throw ValueError("definition name 'self' is reserved");
    ...
    struct RefsGuard {
        ...
        PyObject* GEO;
        PyObject* MSH;
        RefsGuard(PythonManager* manager, PyObject* self)
            : manager(manager),
              PTH(PyObject_GetAttrString(self, "pth")),
              GEO(PyObject_GetAttrString(self, "geo")),
              MSH(PyObject_GetAttrString(self, "msh")) {
            PyObject* globals = manager->globals.ptr();
            PyDict_SetItemString(globals, "PTH", PTH);
            PyDict_SetItemString(globals, "GEO", GEO);
            PyDict_SetItemString(globals, "MSH", MSH);
            Py_DECREF(PTH);
            Py_DECREF(GEO);
            Py_DECREF(MSH);
        }
        ~RefsGuard() {
            PyObject* globals = manager->globals.ptr();
            if (PyDict_GetItemString(globals, "PTH") == PTH) PyDict_DelItemString(globals, "PTH");
            if (PyDict_GetItemString(globals, "GEO") == GEO) PyDict_DelItemString(globals, "GEO");
            if (PyDict_GetItemString(globals, "MSH") == MSH) PyDict_DelItemString(globals, "MSH");
        }
    };
    RefsGuard guard(manager, self.ptr());
    ...
    if (filter.is_none()) {
        ...
        auto filterfun = [sections](const std::string& section) -> bool { return py::extract<bool>(sections.contains(section)); };
...
static shared_ptr<PythonManager> _constructPythonManager(bool draft) { return shared_ptr<PythonManager>(new PythonManager(draft)); }

static py::object constructPythonManager;

static void PythonManager__init__(const py::object& self, bool draft) {
    constructPythonManager(self, draft);
    shared_ptr<PythonManager> manager = py::extract<shared_ptr<PythonManager>>(self);
    manager->makeWeakRef(self);
}
...
void PythonManager::loadDefines(XMLReader& reader) {
    ...
    while (reader.requireTagOrEnd()) {
        ...
        if (!defs.has_key(name)) {
            try {
                ...
                writelog(LOG_WARNING, u8"Cannot parse XML definition '{}' (storing it as string): {}", name,
                         getPythonExceptionMessage());
                PyErr_Clear();
                defs[name] = value;
            }
    ...
    for (py::stl_input_iterator<std::string> key(defs), keys_end; key != keys_end; ++key) {
        try {
            if (parsed.find(*key) == parsed.end()) writelog(LOG_WARNING, "Value '{}' is not defined in the XPL file", *key);
        } catch (py::error_already_set&) {
            PyErr_Clear();
        }
    }
...
shared_ptr<Solver> PythonManager::loadSolver(const std::string& category,
...
void PythonManager::loadMaterialModule(XMLReader& reader) {
    ...
    try {
        ...
        PyErr_Clear();
        throwErrorIfNotDraft(XMLExceptionWithCause(value, reader,
                                                   file.empty()
...
template <typename T> static py::object dict__getitem__(const Manager::Map<T>& self, std::string key) {
    auto found = self.find(key);
    if (found == self.end()) throw KeyError(key);
    return py::object(found->second);
}

template <typename T> static T& dict_get_or_create(Manager::Map<T>& self, std::string key) { return self[key]; }

template <typename T> static void dict__setitem__(Manager::Map<T>& self, std::string key, const T& value) { self[key] = value; }

template <typename T> static void dict__delitem__(Manager::Map<T>& self, std::string key) {
    auto found = self.find(key);
    if (found == self.end()) throw KeyError(key);
    self.erase(found);
}

template <typename T> static size_t dict__len__(const Manager::Map<T>& self) { return self.size(); }

template <typename T> static bool dict__contains__(const Manager::Map<T>& self, const std::string& key) {
    return self.find(key) != self.end();
}

template <typename T> static py::list dict_keys(const Manager::Map<T>& self) {
    py::list result;
    for (auto item : self) {
...
template <typename T> static py::list dict_values(const Manager::Map<T>& self) {
    py::list result;
    for (auto item : self) {
...
template <typename T> static py::list dict_items(const Manager::Map<T>& self) {
    py::list result;
    for (auto item : self) {
...
template <typename T> static py::object dict__getattr__(const Manager::Map<T>& self, const std::string& attr) {
    std::string key = attr;
    auto found = self.find(key);
    if (found == self.end()) {
...
template <typename T> static void dict__setattr__(Manager::Map<T>& self, const std::string& attr, const T& value) {
    std::string key = attr;
    self[key] = value;
}

template <typename T> static void dict__delattr__(Manager::Map<T>& self, const std::string& attr) {
    std::string key = attr;
    auto found = self.find(key);
...
template <typename T> struct dict_iterator {
    const Manager::Map<T>& dict;
    typename Manager::Map<T>::const_iterator i;
    bool is_attr;
    static dict_iterator<T> new_iterator(const Manager::Map<T>& d) { return dict_iterator<T>(d, false); }
    static dict_iterator<T> new_attr_iterator(const Manager::Map<T>& d) { return dict_iterator<T>(d, true); }
...
template <typename T> static inline void register_manager_dict_getters(py::class_<Manager::Map<T>, boost::noncopyable>& c) {
    c.def("__getitem__", &dict__getitem__<T>).def("__getattr__", &dict__getattr__<T>);
}

template <> inline void register_manager_dict_getters(py::class_<Manager::Map<PathHints>, boost::noncopyable>& c) {
    c.def("__getitem__", &dict_get_or_create<PathHints>, py::return_internal_reference<>());
    c.attr("__getattr__") = c.attr("__getitem__");
}

template <typename T> static void register_manager_dict(const std::string name) {
    py::class_<Manager::Map<T>, boost::noncopyable> c((name + "Dict").c_str(),
                                                      (u8"Dictionary holding each loaded " + item_name<T>()).c_str(), py::no_init);
    c.def("__setitem__", &dict__setitem__<T>)
...
static void registerXplPythonGeometryObjectChanger(const std::string& name, py::object changer_class) {
    GeometryReader::registerChangerReader(name, [changer_class](GeometryReader& reader) -> GeometryObject::Changer* {
...
void register_manager() {
    py::class_<Manager, shared_ptr<Manager>, boost::noncopyable> manager("Manager", u8"Main input manager.\n", py::no_init);
    manager.def_readonly("pth", &Manager::pathHints, u8"Dictionary of all named paths.")
    ...
    py::class_<PythonManager, shared_ptr<PythonManager>, py::bases<Manager>, boost::noncopyable>(
        "Manager",
        ...
        py::no_init)
        .def("__init__",
             py::make_function(&PythonManager__init__, py::default_call_policies(), (py::arg("arg1"), py::arg("draft") = false)))
    ...
    constructPythonManager = py::make_constructor(&_constructPythonManager);
}
