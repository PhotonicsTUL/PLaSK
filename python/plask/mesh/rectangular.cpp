...
template <int dim, typename RegisterT> static void register_refined_generator_base(RegisterT& cls) {
    cls.add_property("aspect", &RectangularMeshDivideGenerator<dim>::getAspect,
        ...
        .def_readwrite("warn_multiple", &RectangularMeshDivideGenerator<dim>::warn_multiple,
                       u8"Warn if refining path points to more than one object")
        .def_readwrite("warn_missing", &RectangularMeshDivideGenerator<dim>::warn_missing,
                       u8"Warn if refining path does not point to any object")
        .def_readwrite("warn_ouside", &RectangularMeshDivideGenerator<dim>::warn_outside,
                       u8"Warn if refining line is outside of its object")
    ...
    dividecls.def(
        ...
        py::make_constructor(&RectangularMeshSmoothGenerator__init__<dim>, py::default_call_policies(),
                             (py::arg("small") = py::object(), py::arg("large") = py::object(),
                              py::arg("factor") = py::object(), py::arg("aspect") = 0, py::arg("warn_multiple") = true,
                              py::arg("warn_missing") = true, py::arg("warn_outside") = true)));
