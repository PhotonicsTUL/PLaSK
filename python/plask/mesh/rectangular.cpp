...
#if PY_VERSION_HEX < 0x03060100
        if (PySlice_GetIndicesEx(py::object(slice).ptr(), size, &start, &stop, &step, &length) < 0) throw py::error_already_set();
#else
        if (PySlice_Unpack(py::object(slice).ptr(), &start, &stop, &step) < 0) throw py::error_already_set();
...
template <> struct DivideGeneratorDivMethods<1> {
    static size_t getPre(RectangularMeshDivideGenerator<1>& self) { return self.getPreDivision(Primitive<2>::DIRECTION_TRAN); }

    static size_t getPost(RectangularMeshDivideGenerator<1>& self) { return self.getPostDivision(Primitive<2>::DIRECTION_TRAN); }
...
template <int dim> struct SmoothGeneratorParamMethods {
    ...
    static void setSmall(RectangularMeshSmoothGenerator<dim>& self, py::object val) {
        ...
        // } catch (py::error_already_set) {
        //     PyErr_Clear();
        if (py::len(val) != dim) throw ValueError("wrong size of 'small' ({0} items provided and {1} required)", py::len(val), dim);
    ...
    static void setLarge(RectangularMeshSmoothGenerator<dim>& self, py::object val) {
        ...
        // } catch (py::error_already_set) {
        //     PyErr_Clear();
        if (py::len(val) != dim) throw ValueError("wrong size of 'large' ({0} items provided and {1} required)", py::len(val), dim);
...
template <> struct SmoothGeneratorParamMethods<1> {
    static double getSmall(RectangularMeshSmoothGenerator<1>& self) { return self.getFineStep(Primitive<2>::DIRECTION_TRAN); }

    static double getLarge(RectangularMeshSmoothGenerator<1>& self) { return self.getMaxStep(Primitive<2>::DIRECTION_TRAN); }

    static double getFactor(RectangularMeshSmoothGenerator<1>& self) { return self.getFactor(Primitive<2>::DIRECTION_TRAN); }
...
void RectangularMeshRefinedGenerator_addRefinement1(RectangularMeshDivideGenerator<dim>& self,
    ...
    self.addRefinement(typename Primitive<DIM>::Direction(i), object.shared_from_this(), path, position);
}
...
void RectangularMeshRefinedGenerator_addRefinement2(RectangularMeshDivideGenerator<dim>& self,
    ...
    int i = int(current_axes[axis]) - 3 + DIM;
    if (i < 0 || i > 1) throw ValueError("bad axis name {0}.", axis);
    self.addRefinement(typename Primitive<DIM>::Direction(i), weak_ptr<const GeometryObject>(object.shared_from_this()), position);
}
...
void RectangularMeshRefinedGenerator_removeRefinement1(RectangularMeshDivideGenerator<dim>& self,
    ...
    int i = int(current_axes[axis]) - 3 + DIM;
    if (i < 0 || i > 1) throw ValueError("bad axis name {0}.", axis);
    self.removeRefinement(typename Primitive<DIM>::Direction(i), object.shared_from_this(), path, position);
}
...
void RectangularMeshRefinedGenerator_removeRefinement2(RectangularMeshDivideGenerator<dim>& self,
    ...
    int i = int(current_axes[axis]) - 3 + DIM;
    if (i < 0 || i > 1) throw ValueError("bad axis name {0}.", axis);
    self.removeRefinement(typename Primitive<DIM>::Direction(i), weak_ptr<const GeometryObject>(object.shared_from_this()),
                          position);
}
...
void RectangularMeshRefinedGenerator_removeRefinements1(RectangularMeshDivideGenerator<dim>& self,
                                                        const GeometryObject& object,
                                                        const PathHints& path) {
    self.removeRefinements(object.shared_from_this(), path);
}
...
py::dict RectangularMeshRefinedGenerator_listRefinements(const RectangularMeshDivideGenerator<dim>& self, const std::string& axis) {
    ...
    for (auto refinement : self.getRefinements(typename Primitive<DIM>::Direction(i))) {
        py::object object{const_pointer_cast<GeometryObject>(refinement.first.first.lock())};
...
template <int dim, typename RegisterT> static void register_refined_generator_base(RegisterT& cls) {
    cls.add_property("aspect", &RectangularMeshDivideGenerator<dim>::getAspect, &RectangularMeshDivideGenerator<dim>::setAspect,
...
shared_ptr<RectangularMeshDivideGenerator<dim>> RectangularMeshDivideGenerator__init__(py::object prediv,
    ...
    if (gradual.ptr() == Py_True)
        ...
        result->gradual = 0;
        for (int i = 0; i != dim; ++i) result->setGradual(i, py::extract<bool>(gradual[i]));
    }
    result->aspect = aspect;
    return result;
};

template <int dim> py::object RectangularMeshDivideGenerator_getGradual(const RectangularMeshDivideGenerator<dim>& self);

template <> py::object RectangularMeshDivideGenerator_getGradual<1>(const RectangularMeshDivideGenerator<1>& self) {
    return py::object(bool(self.gradual));
}

template <> py::object RectangularMeshDivideGenerator_getGradual<2>(const RectangularMeshDivideGenerator<2>& self) {
    return py::make_tuple(self.getGradual(0), self.getGradual(1));
}

template <> py::object RectangularMeshDivideGenerator_getGradual<3>(const RectangularMeshDivideGenerator<3>& self) {
    return py::make_tuple(self.getGradual(0), self.getGradual(1), self.getGradual(2));
}

template <int dim> void RectangularMeshDivideGenerator_setGradual(RectangularMeshDivideGenerator<dim>& self, py::object value) {
    if (value.ptr() == Py_True) {
        ...
        self.gradual = 0;
        for (int i = 0; i != dim; ++i) self.setGradual(i, py::extract<bool>(value[i]));
    }
}

template <int dim> void register_divide_generator() {
    py::class_<RectangularMeshDivideGenerator<dim>, shared_ptr<RectangularMeshDivideGenerator<dim>>, py::bases<MeshGeneratorD<dim>>,
               boost::noncopyable>
    ...
    dividecls
        ...
        .add_property("gradual", &RectangularMeshDivideGenerator_getGradual<dim>, &RectangularMeshDivideGenerator_setGradual<dim>,
...
static shared_ptr<RectangularMesh2DRegularGenerator> RectangularMesh2DRegularGenerator__init__2(double spacing0,
...
template <int dim> void register_smooth_generator() {
    py::class_<RectangularMeshSmoothGenerator<dim>, shared_ptr<RectangularMeshSmoothGenerator<dim>>, py::bases<MeshGeneratorD<dim>>,
               boost::noncopyable>
        dividecls("SmoothGenerator",
                  ...
                  py::no_init);
    register_refined_generator_base<dim>(dividecls);
    dividecls.def("__init__", py::make_constructor(&RectangularMeshSmoothGenerator__init__<dim>, py::default_call_policies(),
...
void register_mesh_rectangular() {
    ...
    rectilinear1d.def("__init__", py::make_constructor(&__init__empty<OrderedAxis>))
        .def("__init__",
             py::make_constructor(&Rectilinear__init__seq<OrderedAxis>, py::default_call_policies(), (py::arg("points"))))
...

    {
        ...
        py::class_<OrderedMesh1DSimpleGenerator, shared_ptr<OrderedMesh1DSimpleGenerator>, py::bases<MeshGeneratorD<1>>,
                   boost::noncopyable>("SimpleGenerator",
        ...
        py::class_<OrderedMesh1DRegularGenerator, shared_ptr<OrderedMesh1DRegularGenerator>, py::bases<MeshGeneratorD<1>>,
                   boost::noncopyable>("RegularGenerator",
                                       u8"Generator of ordered 1D mesh with lines at transverse edges of all objects\n"
    ...
    py::class_<RegularAxis, shared_ptr<RegularAxis>, py::bases<MeshAxis>>(
        ...
        .def("__init__",
             py::make_constructor(&Regular__init__one_param<RegularAxis>, py::default_call_policies(), (py::arg("value"))))
    ...
    py::class_<RectangularMeshBase2D, shared_ptr<RectangularMeshBase2D>, py::bases<MeshD<2>>, boost::noncopyable> rectangularBase2D(
        "RectangularBase2D",
    ...
    rectangularBase2D
        ...
        .staticmethod("Left")
        .def("Right", &RectangularMeshBase2D::getRightBoundary, u8"Right edge of the mesh for setting boundary conditions")
        .staticmethod("Right")
        .def("Top", &RectangularMeshBase2D::getTopBoundary, u8"Top edge of the mesh for setting boundary conditions")
        .staticmethod("Top")
        .def("Bottom", &RectangularMeshBase2D::getBottomBoundary, u8"Bottom edge of the mesh for setting boundary conditions")
        .staticmethod("Bottom")
        .def("LeftOf",
             (RectangularMeshBase2D::Boundary (*)(shared_ptr<const GeometryObject>,
                                                  const PathHints&))&RectangularMeshBase2D::getLeftOfBoundary,
             u8"Boundary left of specified object", (py::arg("object"), py::arg("path") = py::object()))
        .staticmethod("LeftOf")
        .def("RightOf",
             (RectangularMeshBase2D::Boundary (*)(shared_ptr<const GeometryObject>,
                                                  const PathHints&))&RectangularMeshBase2D::getRightOfBoundary,
             u8"Boundary right of specified object", (py::arg("object"), py::arg("path") = py::object()))
        .staticmethod("RightOf")
        .def("TopOf",
             (RectangularMeshBase2D::Boundary (*)(shared_ptr<const GeometryObject>,
                                                  const PathHints&))&RectangularMeshBase2D::getTopOfBoundary,
             u8"Boundary top of specified object", (py::arg("object"), py::arg("path") = py::object()))
        .staticmethod("TopOf")
        .def("BottomOf",
             (RectangularMeshBase2D::Boundary (*)(shared_ptr<const GeometryObject>,
                                                  const PathHints&))&RectangularMeshBase2D::getBottomOfBoundary,
             u8"Boundary bottom of specified object", (py::arg("object"), py::arg("path") = py::object()))
        .staticmethod("BottomOf")
        .def("Horizontal",
             (RectangularMeshBase2D::Boundary (*)(double, double, double))&RectangularMeshBase2D::getHorizontalBoundaryNear,
             u8"Boundary at horizontal line", (py::arg("at"), "start", "stop"))
        .def("Horizontal", (RectangularMeshBase2D::Boundary (*)(double))&RectangularMeshBase2D::getHorizontalBoundaryNear,
             u8"Boundary at horizontal line", py::arg("at"))
        .staticmethod("Horizontal")
        .def("Vertical",
             (RectangularMeshBase2D::Boundary (*)(double, double, double))&RectangularMeshBase2D::getVerticalBoundaryNear,
    ...
    py::class_<RectangularMesh<2>, shared_ptr<RectangularMesh<2>>, py::bases<RectangularMeshBase2D>> rectangular2D(
        ...
        py::no_init);
    rectangular2D
        .def("__init__", py::make_constructor(&RectangularMesh2D__init__empty<RectangularMesh<2>>, py::default_call_policies(),
                                              (py::arg("ordering") = "01")))
        ...
        .def("index", (size_t (RectangularMesh<2>::*)(size_t, size_t) const) & RectangularMesh<2>::index,
             u8"Return single index of the point indexed with index0 and index1", (py::arg("index0"), py::arg("index1")))
        ...
        .def("major_index", &RectangularMesh<2>::majorIndex, u8"Return index in the major axis of the point with given index",
             (py::arg("index")))
        .def("minor_index", &RectangularMesh<2>::minorIndex, u8"Return index in the minor axis of the point with given index",
             (py::arg("index")))
        ...
        .add_property("elements", py::make_function(&RectangularMesh2D::elements, py::with_custodian_and_ward_postcall<0, 1>()),
...

    {
        ...
        py::class_<RectangularMesh2DSimpleGenerator, shared_ptr<RectangularMesh2DSimpleGenerator>, py::bases<MeshGeneratorD<2>>,
                   boost::noncopyable>("SimpleGenerator",
        ...
        py::class_<RectangularMesh2DRegularGenerator, shared_ptr<RectangularMesh2DRegularGenerator>, py::bases<MeshGeneratorD<2>>,
                   boost::noncopyable>("RegularGenerator",
    ...
    py::class_<RectangularMeshBase3D, shared_ptr<RectangularMeshBase3D>, py::bases<MeshD<3>>, boost::noncopyable> rectangularBase3D(
        "RectangularBase3D",
        u8"Base class for 3D rectangular meshes."
        u8"Do not use it directly.",
        py::no_init);
    rectangularBase3D
        .def("Front", &RectangularMeshBase3D::getFrontBoundary, u8"Front side of the mesh for setting boundary conditions")
        .staticmethod("Front")
        ...
        .staticmethod("Left")
        .def("Right", &RectangularMeshBase3D::getRightBoundary, u8"Right side of the mesh for setting boundary conditions")
        .staticmethod("Right")
        .def("Top", &RectangularMeshBase3D::getTopBoundary, u8"Top side of the mesh for setting boundary conditions")
        .staticmethod("Top")
        .def("Bottom", &RectangularMeshBase3D::getBottomBoundary, u8"Bottom side of the mesh for setting boundary conditions")
        .staticmethod("Bottom")
        .def("FrontOf",
             (RectangularMeshBase3D::Boundary (*)(shared_ptr<const GeometryObject>,
                                                  const PathHints&))&RectangularMesh<3>::getFrontOfBoundary,
             u8"Boundary in front of specified object", (py::arg("object"), py::arg("path") = py::object()))
        .staticmethod("FrontOf")
        .def("BackOf",
             (RectangularMeshBase3D::Boundary (*)(shared_ptr<const GeometryObject>,
                                                  const PathHints&))&RectangularMesh<3>::getBackOfBoundary,
             u8"Boundary back of specified object", (py::arg("object"), py::arg("path") = py::object()))
        .staticmethod("BackOf")
        .def("LeftOf",
             (RectangularMeshBase3D::Boundary (*)(shared_ptr<const GeometryObject>,
                                                  const PathHints&))&RectangularMesh<3>::getLeftOfBoundary,
             u8"Boundary left of specified object", (py::arg("object"), py::arg("path") = py::object()))
        .staticmethod("LeftOf")
        .def("RightOf",
             (RectangularMeshBase3D::Boundary (*)(shared_ptr<const GeometryObject>,
                                                  const PathHints&))&RectangularMesh<3>::getRightOfBoundary,
             u8"Boundary right of specified object", (py::arg("object"), py::arg("path") = py::object()))
        .staticmethod("RightOf")
        .def("TopOf",
             (RectangularMeshBase3D::Boundary (*)(shared_ptr<const GeometryObject>,
                                                  const PathHints&))&RectangularMesh<3>::getTopOfBoundary,
             u8"Boundary top of specified object", (py::arg("object"), py::arg("path") = py::object()))
        .staticmethod("TopOf")
        .def("BottomOf",
             (RectangularMeshBase3D::Boundary (*)(shared_ptr<const GeometryObject>,
                                                  const PathHints&))&RectangularMesh<3>::getBottomOfBoundary,
             u8"Boundary bottom of specified object", (py::arg("object"), py::arg("path") = py::object()))
        .staticmethod("BottomOf");
    ...
    py::class_<RectangularMesh<3>, shared_ptr<RectangularMesh<3>>, py::bases<RectangularMeshBase3D>> rectangular3D(
        ...
        py::no_init);
    rectangular3D
        .def("__init__", py::make_constructor(&RectangularMesh3D__init__empty<RectangularMesh<3>>, py::default_call_policies(),
                                              (py::arg("ordering") = "012")))
        ...
        .def("major_index", &RectangularMesh<3>::majorIndex, u8"Return index in the major axis of the point with given index",
             (py::arg("index")))
        .def("middle_index", &RectangularMesh<3>::middleIndex, u8"Return index in the middle axis of the point with given index",
             (py::arg("index")))
        .def("minor_index", &RectangularMesh<3>::minorIndex, u8"Return index in the minor axis of the point with given index",
             (py::arg("index")))
        ...
        .add_property("ordering", &RectangularMesh3D__getOrdering, &RectangularMesh3D__setOrdering,
                      u8"Ordering of the points in this mesh")
        .add_property("elements", py::make_function(&RectangularMesh3D_elements, py::with_custodian_and_ward_postcall<0, 1>()),
                      u8"Element list in the mesh")
...

    {
        ...
        py::class_<RectangularMesh3DSimpleGenerator, shared_ptr<RectangularMesh3DSimpleGenerator>, py::bases<MeshGeneratorD<3>>,
                   boost::noncopyable>("SimpleGenerator",
        ...
        py::class_<RectangularMesh3DRegularGenerator, shared_ptr<RectangularMesh3DRegularGenerator>, py::bases<MeshGeneratorD<3>>,
                   boost::noncopyable>("RegularGenerator",
            ...
            .def("__init__", py::make_constructor(RectangularMesh3DRegularGenerator__init__1, py::default_call_policies(),
                                                  (py::arg("spacing"), py::arg("split") = false)))
            .def("__init__",
                 py::make_constructor(RectangularMesh3DRegularGenerator__init__3, py::default_call_policies(),
