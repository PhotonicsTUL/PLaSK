...
InGaN_Mg::InGaN_Mg(const Material::Composition& Comp, double Val) : InGaN(Comp), mGaN_Mg(Val), mInN_Mg(Val) { NA = Val; }

MI_PROPERTY(InGaN_Mg,
            mob,
...
MI_PROPERTY(InGaN_Mg, Nf, MINote("linear interpolation: Mg-doped GaN, InN"))
double InGaN_Mg::Nf(double T) const { return (mInN_Mg.Nf(T) * In + mGaN_Mg.Nf(T) * Ga); }

MI_PROPERTY(InGaN_Mg, Na, MINote("-"))
double InGaN_Mg::Na() const { return (NA); }

MI_PROPERTY(InGaN_Mg, Nd, MINote("-"))
double InGaN_Mg::Nd() const { return (0.); }

double InGaN_Mg::doping() const { return NA; }
...
MI_PROPERTY(InGaN_Mg, absp, MISeeClass<InGaN>(MaterialInfo::absp))
double InGaN_Mg::absp(double lam, double T) const {
    double E = phys::h_eVc1e9 / lam;
    return ((19000. + 200. * doping() / 1e18) * exp((E - Eg(T, 0., 'G')) / (0.019 + 0.0001 * doping() / 1e18)) +
            (330. + 30. * doping() / 1e18) * exp((E - Eg(T, 0., 'G')) / (0.07 + 0.0008 * doping() / 1e18)));
}
