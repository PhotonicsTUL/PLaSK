...
InGaN_Si::InGaN_Si(const Material::Composition& Comp, double Val) : InGaN(Comp), mGaN_Si(Val), mInN_Si(Val) { ND = Val; }

MI_PROPERTY(InGaN_Si,
            mob,
...
MI_PROPERTY(InGaN_Si, Nf, MINote("linear interpolation: Si-doped GaN, InN"))
double InGaN_Si::Nf(double T) const { return (mInN_Si.Nf(T) * In + mGaN_Si.Nf(T) * Ga); }

MI_PROPERTY(InGaN_Si, Na, MINote("-"))
double InGaN_Si::Na() const { return (0.); }

MI_PROPERTY(InGaN_Si, Nd, MINote("-"))
double InGaN_Si::Nd() const { return (ND); }

double InGaN_Si::doping() const { return ND; }
...
MI_PROPERTY(InGaN_Si, thermk, MISeeClass<InGaN>(MaterialInfo::thermk), MINote("Si doping dependence for GaN"))
...
MI_PROPERTY(InGaN_Si, absp, MISeeClass<InGaN>(MaterialInfo::absp))
double InGaN_Si::absp(double lam, double T) const {
    double E = phys::h_eVc1e9 / lam;
    return ((19000. + 4000. * doping() / 1e18) * exp((E - Eg(T, 0., 'G')) / (0.019 + 0.001 * doping() / 1e18)) +
            (330. + 200. * doping() / 1e18) * exp((E - Eg(T, 0., 'G')) / (0.07 + 0.016 * doping() / 1e18)));
}
