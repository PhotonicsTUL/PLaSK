...
OldBesselSolverCyl::OldBesselSolverCyl(const std::string& name):
    ...
    outLoss(this, &OldBesselSolverCyl::getModalLoss,  &OldBesselSolverCyl::nummodes)
{
    pml.dist = 20.;
    pml.size = 0.;
...
void OldBesselSolverCyl::loadConfiguration(XMLReader& reader, Manager& manager)
{
...
void OldBesselSolverCyl::onInitialize()
{
    this->setupLayers();
    if (this->interface == -1)
    ...
    switch (domain) {
        case DOMAIN_FINITE:
            expansion.reset(new ExpansionOldBesselFini(this));
            break;
        case DOMAIN_INFINITE:
            expansion.reset(new ExpansionOldBesselInfini(this));
            break;
        default:
            assert(0);
    }
...
void OldBesselSolverCyl::onInvalidate()
{
    modes.clear();
    expansion->reset();
    transfer.reset();
...
size_t OldBesselSolverCyl::findMode(dcomplex start, int m)
{
...
LazyData<Vec<3,dcomplex>> OldBesselSolverCyl::getE(size_t num, shared_ptr<const MeshD<2>> dst_mesh, InterpolationMethod method)
{
...
LazyData<Vec<3,dcomplex>> OldBesselSolverCyl::getH(size_t num, shared_ptr<const MeshD<2>> dst_mesh, InterpolationMethod method)
{
...
LazyData<double> OldBesselSolverCyl::getMagnitude(size_t num, shared_ptr<const MeshD<2>> dst_mesh, InterpolationMethod method)
{
...
double OldBesselSolverCyl::getWavelength(size_t n) {
...
#ifndef NDEBUG
cmatrix OldBesselSolverCyl::epsVmm(size_t layer) {
    Solver::initCalculation();
    computeIntegrals();
    return expansion->epsVmm(layer);
}
cmatrix OldBesselSolverCyl::epsVpp(size_t layer) {
    Solver::initCalculation();
    computeIntegrals();
    return expansion->epsVpp(layer);
}
cmatrix OldBesselSolverCyl::epsTmm(size_t layer) {
    Solver::initCalculation();
    computeIntegrals();
    return expansion->epsTmm(layer);
}
cmatrix OldBesselSolverCyl::epsTpp(size_t layer) {
    Solver::initCalculation();
    computeIntegrals();
    return expansion->epsTpp(layer);
}
cmatrix OldBesselSolverCyl::epsTmp(size_t layer) {
    Solver::initCalculation();
    computeIntegrals();
    return expansion->epsTmp(layer);
}
cmatrix OldBesselSolverCyl::epsTpm(size_t layer) {
    Solver::initCalculation();
    computeIntegrals();
    return expansion->epsTpm(layer);
}
cmatrix OldBesselSolverCyl::epsDm(size_t layer) {
    Solver::initCalculation();
    computeIntegrals();
    return expansion->epsDm(layer);
}
cmatrix OldBesselSolverCyl::epsDp(size_t layer) {
    Solver::initCalculation();
    computeIntegrals();
    return expansion->epsDp(layer);
}
dmatrix OldBesselSolverCyl::epsVV(size_t layer) {
    Solver::initCalculation();
    computeIntegrals();
    return expansion->epsVV(layer);
}

cmatrix OldBesselSolverCyl::muVmm() {
    Solver::initCalculation();
    if (auto finite_expansion = dynamic_cast<ExpansionOldBesselFini*>(expansion.get())) {
        computeIntegrals();
...
cmatrix OldBesselSolverCyl::muVpp() {
    Solver::initCalculation();
    if (auto finite_expansion = dynamic_cast<ExpansionOldBesselFini*>(expansion.get())) {
        computeIntegrals();
...
cmatrix OldBesselSolverCyl::muTmm() {
    Solver::initCalculation();
    if (auto finite_expansion = dynamic_cast<ExpansionOldBesselFini*>(expansion.get())) {
        computeIntegrals();
...
cmatrix OldBesselSolverCyl::muTpp() {
    Solver::initCalculation();
    if (auto finite_expansion = dynamic_cast<ExpansionOldBesselFini*>(expansion.get())) {
        computeIntegrals();
...
cmatrix OldBesselSolverCyl::muTmp() {
    Solver::initCalculation();
    if (auto finite_expansion = dynamic_cast<ExpansionOldBesselFini*>(expansion.get())) {
        computeIntegrals();
...
cmatrix OldBesselSolverCyl::muTpm() {
    Solver::initCalculation();
    if (auto finite_expansion = dynamic_cast<ExpansionOldBesselFini*>(expansion.get())) {
        computeIntegrals();
...
cmatrix OldBesselSolverCyl::muDm() {
    Solver::initCalculation();
    if (auto finite_expansion = dynamic_cast<ExpansionOldBesselFini*>(expansion.get())) {
        computeIntegrals();
...
cmatrix OldBesselSolverCyl::muDp() {
    Solver::initCalculation();
    if (auto finite_expansion = dynamic_cast<ExpansionOldBesselFini*>(expansion.get())) {
        computeIntegrals();
...
dmatrix OldBesselSolverCyl::muVV() {
    Solver::initCalculation();
    if (auto finite_expansion = dynamic_cast<ExpansionOldBesselFini*>(expansion.get())) {
        computeIntegrals();
