...
ExpansionOldBessel::ExpansionOldBessel(OldBesselSolverCyl* solver): Expansion(solver), m(1),
                                                           initialized(false), m_changed(true)
{
}


size_t ExpansionOldBessel::matrixSize() const {
    return 2 * SOLVER->size; // TODO should be N for m = 0?
}


void ExpansionOldBessel::init1()
{
    // Initialize segments
    if (SOLVER->mesh)
...
void ExpansionOldBessel::prepareIntegrals(double lam, double glam) {
    if (m_changed) init2();
...
void ExpansionOldBessel::cleanupIntegrals(double, double) {
    temperature.reset();
    gain.reset();
}

std::pair<dcomplex, dcomplex> ExpansionOldBessel::integrateLayer(size_t layer, double lam, double glam, bool finite) {
...
#ifndef NDEBUG
cmatrix ExpansionOldBessel::epsVmm(size_t layer) {
...
cmatrix ExpansionOldBessel::epsVpp(size_t layer) {
    size_t N = SOLVER->size;
    cmatrix result(N, N, 0.);
    for (size_t i = 0; i != N; ++i)
        for (size_t j = 0; j != N; ++j)
...
cmatrix ExpansionOldBessel::epsTmm(size_t layer) {
    size_t N = SOLVER->size;
    cmatrix result(N, N, 0.);
    for (size_t i = 0; i != N; ++i)
        for (size_t j = 0; j != N; ++j)
...
cmatrix ExpansionOldBessel::epsTpp(size_t layer) {
    size_t N = SOLVER->size;
    cmatrix result(N, N, 0.);
    for (size_t i = 0; i != N; ++i)
        for (size_t j = 0; j != N; ++j)
...
cmatrix ExpansionOldBessel::epsTmp(size_t layer) {
    size_t N = SOLVER->size;
    cmatrix result(N, N, 0.);
    for (size_t i = 0; i != N; ++i)
        for (size_t j = 0; j != N; ++j)
...
cmatrix ExpansionOldBessel::epsTpm(size_t layer) {
    size_t N = SOLVER->size;
    cmatrix result(N, N, 0.);
    for (size_t i = 0; i != N; ++i)
        for (size_t j = 0; j != N; ++j)
...
cmatrix ExpansionOldBessel::epsDm(size_t layer) {
    size_t N = SOLVER->size;
    cmatrix result(N, N, 0.);
    for (size_t i = 0; i != N; ++i)
        for (size_t j = 0; j != N; ++j)
...
cmatrix ExpansionOldBessel::epsDp(size_t layer) {
    size_t N = SOLVER->size;
    cmatrix result(N, N, 0.);
    for (size_t i = 0; i != N; ++i)
        for (size_t j = 0; j != N; ++j)
...
dmatrix ExpansionOldBessel::epsVV(size_t layer) {
    size_t N = SOLVER->size;
...
void ExpansionOldBessel::prepareField()
{
    if (field_interpolation == INTERPOLATION_DEFAULT) field_interpolation = INTERPOLATION_NEAREST;
}

void ExpansionOldBessel::cleanupField()
{
}

LazyData<Vec<3,dcomplex>> ExpansionOldBessel::getField(size_t l,
...
LazyData<Tensor3<dcomplex>> ExpansionOldBessel::getMaterialNR(size_t layer,
                                    const shared_ptr<const typename LevelsAdapter::Level>& level,
