 ...
 */
#ifndef PLASK__SOLVER__OPTICAL__SLAB_LINES_REFLECTION_2D_H
#define PLASK__SOLVER__OPTICAL__SLAB_LINES_REFLECTION_2D_H
 ...
 */
struct PLASK_SOLVER_API LinesSolver2D: public SlabSolver<SolverWithMesh<Geometry2DCartesian,RegularAxis>> {

    friend struct ExpansionFD2D;

    std::string getClassName() const override { return "optical.Lines2D"; }
    ...
    struct Mode {
        ...
        Mode(const ExpansionFD2D& expansion, double tolx):
        ...
        bool operator==(const ExpansionFD2D& other) const {
            return is_equal(k0, other.k0) && is_equal(beta, other.beta) && is_equal(ktran, other.ktran)
                && symmetry == other.symmetry && polarization == other.polarization &&
                ((isnan(lam0) && isnan(other.lam0)) || lam0 == other.lam0)
            ;
  ...
  public:
    ...
    LinesSolver2D(const std::string& name="");
     ...
     */
    size_t findMode(LinesSolver2D::What what, dcomplex start);

    /// Get automatic mesh density
    size_t getDensity() const { return density; }
    /// Set automatic mesh density
    void setDensity(double dens) {
        density = dens;
        invalidate();
    }

    void setMesh(const shared_ptr<RegularAxis>& mesh) {
        SlabSolver<SolverWithMesh<Geometry2DCartesian,RegularAxis>>::setMesh(mesh);
        density = NAN;
    }

    void setMesh(shared_ptr<MeshGeneratorD<1>> generator) {
        SlabSolver<SolverWithMesh<Geometry2DCartesian,RegularAxis>>::setMesh(generator);
        density = NAN;
    }
    ...
    RegularAxis getMesh() { initCalculation(); return *expansion.mesh; }

    const PML& getPML() { initCalculation(); return expansion.pml; }
