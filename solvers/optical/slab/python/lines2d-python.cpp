...
#include "lines2d-python.hpp"
#include "slab-python.hpp"

namespace plask { namespace optical { namespace slab { namespace python {

template <> inline const char* solver_compute_reflectivity_name<LinesSolver2D>() { return "Lines2D.compute_reflectivity"; }

template <> inline const char* solver_compute_transmittivity_name<LinesSolver2D>() { return "Lines2D.compute_transmittivity"; }

template <> py::object Eigenmodes<LinesSolver2D>::array(const dcomplex* data, size_t N) const {
    int dim = 2, strid = 2;
...
static py::object LinesSolver2D_getMirrors(const LinesSolver2D& self) {
    if (!self.mirrors) return py::object();
    return py::make_tuple(self.mirrors->first, self.mirrors->second);
}

static void LinesSolver2D_setMirrors(LinesSolver2D& self, py::object value) {
    if (value.is_none())
        self.mirrors.reset();
    else {
        try {
...
static py::object LinesSolver2D_getDeterminant(py::tuple args, py::dict kwargs) {
    if (py::len(args) != 1)
        throw TypeError(u8"get_determinant() takes exactly one non-keyword argument ({0} given)", py::len(args));
    LinesSolver2D* self = py::extract<LinesSolver2D*>(args[0]);
    ...
    switch (what) {
        ...
        case WHAT_WAVELENGTH:
            return UFUNC<dcomplex>(
                [self, neff](dcomplex x) -> dcomplex {
                    self->expansion.setK0(2e3 * PI / x);
                    if (neff) self->expansion.setBeta(*neff * self->expansion.k0);
                    return self->getDeterminant();
                },
                array, "Lines2D.get_determinant", "lam");
        case WHAT_K0:
            return UFUNC<dcomplex>(
                [self, neff](dcomplex x) -> dcomplex {
                    self->expansion.setK0(x);
                    if (neff) self->expansion.setBeta(*neff * x);
                    return self->getDeterminant();
                },
                array, "Lines2D.get_determinant", "k0");
        case WHAT_NEFF:
            return UFUNC<dcomplex>(
                [self](dcomplex x) -> dcomplex {
                    self->expansion.setBeta(x * self->getK0());
                    return self->getDeterminant();
                },
                array, "Lines2D.get_determinant", "neff");
        case WHAT_KTRAN:
            return UFUNC<dcomplex>(
                [self](dcomplex x) -> dcomplex {
                    self->expansion.setKtran(x);
                    return self->getDeterminant();
                },
                array, "Lines2D.get_determinant", "ktran");
    }
    return py::object();
}

static size_t LinesSolver2D_setMode(py::tuple args, py::dict kwargs) {
    if (py::len(args) != 1) throw TypeError(u8"set_mode() takes exactly one non-keyword argument ({0} given)", py::len(args));
    LinesSolver2D* self = py::extract<LinesSolver2D*>(args[0]);
    auto* expansion = &self->expansion;
    ...
    if (k0)
        ...
        expansion->setK0(self->getK0());
    if (neff)
        expansion->setBeta(*neff * expansion->k0);
    else
        expansion->setBeta(self->getBeta());
    if (ktran)
        expansion->setKtran(*ktran);
...
static size_t LinesSolver2D_findMode(py::tuple args, py::dict kwargs) {
    if (py::len(args) != 1) throw TypeError(u8"find_mode() takes exactly one non-keyword argument ({0} given)", py::len(args));
    LinesSolver2D* self = py::extract<LinesSolver2D*>(args[0]);

    if (py::len(kwargs) != 1) throw TypeError(u8"find_mode() takes exactly one keyword argument ({0} given)", py::len(kwargs));
    std::string key = py::extract<std::string>(kwargs.keys()[0]);
    dcomplex value = py::extract<dcomplex>(kwargs[key]);
    AxisNames* axes = getCurrentAxes();
    LinesSolver2D::What what;

    if (key == "lam" || key == "wavelength")
        what = LinesSolver2D::WHAT_WAVELENGTH;
    else if (key == "k0")
        what = LinesSolver2D::WHAT_K0;
    else if (key == "neff")
        what = LinesSolver2D::WHAT_NEFF;
    else if (key == "ktran" || key == "kt" || key == "k" + axes->getNameForTran())
        what = LinesSolver2D::WHAT_KTRAN;
    else
...
static dcomplex LinesSolver2D_Mode_Neff(const LinesSolver2D::Mode& mode) { return mode.beta / mode.k0; }

static py::object LinesSolver2D_Mode__getattr__(const LinesSolver2D::Mode& mode, const std::string name) {
...
static std::string LinesSolver2D_Mode_str(const LinesSolver2D::Mode& self) {
    AxisNames* axes = getCurrentAxes();
    std::string pol;
    ...
    return format(u8"<lam: {:.2f}nm, neff: {}, ktran: {}/um, polarization: {}, symmetry: {}, power: {:.2g} mW>",
                  real(2e3 * PI / self.k0), str(self.beta / self.k0, "{:.3f}{:+.3g}j"),
                  str(self.ktran, "({:.3g}{:+.3g}j)", "{:.3g}"), pol, sym, self.power);
}
static std::string LinesSolver2D_Mode_repr(const LinesSolver2D::Mode& self) {
    AxisNames* axes = getCurrentAxes();
    std::string pol;
    switch (self.polarization) {
        case Expansion::E_TRAN: pol = "'E" + axes->getNameForTran() + "'"; break;
    ...
    return format(u8"Lines2D.Mode(lam={0}, neff={1}, ktran={2}, polarization={3}, symmetry={4}, power={5})",
                  str(2e3 * PI / self.k0), str(self.beta / self.k0), str(self.ktran), pol, sym, self.power);
}

static py::object LinesSolver2D_getFieldVectorE(LinesSolver2D& self, int num, double z) {
...
static py::object LinesSolver2D_getFieldVectorH(LinesSolver2D& self, int num, double z) {
    if (num < 0) num += int(self.modes.size());
    if (std::size_t(num) >= self.modes.size()) throw IndexError(u8"Bad mode number {:d}", num);
    return arrayFromVec2D<NPY_CDOUBLE>(self.getFieldVectorH(num, z), self.separated(), 2);
}

// static py::object LinesSolver2D_getReflectedFieldVectorE(LinesSolver2D::Reflected& self, double z) {
...
// }
//
// static py::object LinesSolver2D_getReflectedFieldVectorH(LinesSolver2D::Reflected& self, double z) {
//     return arrayFromVec2D<NPY_CDOUBLE>(self.parent->getScatteredFieldVectorH(self.polarization, self.side, z),
//     self.parent->separated(), 2);
// }

static py::object LinesSolver2D_incidentGaussian(LinesSolver2D& self,
                                                 Transfer::IncidentDirection side,
                                                 Expansion::Component polarization,
                                                 double sigma,
                                                 double center) {
    return arrayFromVec<NPY_CDOUBLE>(self.incidentGaussian(side, polarization, sigma, center));
}

static shared_ptr<Scattering<LinesSolver2D>> LinesSolver2D_scatteringGaussian(LinesSolver2D& self,
                                                                              Transfer::IncidentDirection side,
                                                                              Expansion::Component polarization,
                                                                              double sigma,
                                                                              double center) {
    return shared_ptr<Scattering<LinesSolver2D>>(
        new Scattering<LinesSolver2D>(&self, side, self.incidentGaussian(side, polarization, sigma, center)));
}

py::object LinesSolver2D_getDensity(const LinesSolver2D& self) {
    double density = self.getDensity();
    if (isnan(density)) return py::object();
    return py::object(density);
}

void LinesSolver2D_setDensity(LinesSolver2D& self, const py::object& density) {
    if (density.is_none())
        self.setDensity(NAN);
    else
        self.setDensity(py::extract<double>(density));
}

PmlWrapper LinesSolver2D_getPML(LinesSolver2D& self) {
    return PmlWrapper(self.getPML());
}

void export_LinesSolver2D() {
    CLASS(LinesSolver2D, "Lines2D",
          u8"Optical Solver using Lines expansion in 2D.\n\n"
          u8"It calculates optical modes and optical field distribution using Lines slab method\n"
          u8"and reflection transfer in two-dimensional Cartesian space.")
    export_base(solver);
    PROVIDER(outNeff, "Effective index of the last computed modOrthogonal expane.");
    solver.add_property("density", &LinesSolver2D_getDensity, &LinesSolver2D_setDensity, "Density of the auto-generated mesh.");
    ...
    solver.add_property("actual_mesh", &__Class__::getMesh,
                        "Horizontal mesh horizontal at which computations are performed.");
    solver.add_property("actual_pml", &LinesSolver2D_getPML,
                        "Horizontal PMLs used for computations (may be different than the defined ones).");
    solver.def("get_determinant", py::raw_function(LinesSolver2D_getDeterminant),
    ...
    solver.def("find_mode", py::raw_function(LinesSolver2D_findMode),
               ...
               u8"    neff (complex): Longitudinal effective index.\n"
               u8"    ktran (complex): Transverse wavevector.\n");
    solver.def("set_mode", py::raw_function(LinesSolver2D_setMode),
               ...
               u8"    neff (complex): Longitudinal effective index.\n"
               u8"    ktran (complex): Transverse wavevector.\n");
    solver.def("compute_reflectivity", &Solver_computeReflectivity_polarization<LinesSolver2D>,
               (py::arg("lam"), "side", "polarization"));
    solver.def("compute_reflectivity", &Solver_computeReflectivity_index<LinesSolver2D>, (py::arg("lam"), "side", "index"));
    solver.def("compute_reflectivity", &Solver_computeReflectivity_array<LinesSolver2D>, (py::arg("lam"), "side", "coffs"),
    ...
    solver.def("compute_transmittivity", &Solver_computeTransmittivity_polarization<LinesSolver2D>,
               (py::arg("lam"), "side", "polarization"));
    solver.def("compute_transmittivity", &Solver_computeTransmittivity_index<LinesSolver2D>, (py::arg("lam"), "side", "index"));
    solver.def("compute_transmittivity", &Solver_computeTransmittivity_array<LinesSolver2D>, (py::arg("lam"), "side", "coffs"),
               ...
               u8"    idx: Eigenmode number.\n"
               u8"    coeffs: expansion coefficients of the incident vector.\n");
    solver.add_property("mirrors", LinesSolver2D_getMirrors, LinesSolver2D_setMirrors,
                        u8"Mirror reflectivities. If None then they are automatically estimated from the\n"
                        u8"Fresnel equations.");
    solver.add_property("pml", py::make_function(&Solver_getPML<LinesSolver2D>, py::with_custodian_and_ward_postcall<0, 1>()),
                        &Solver_setPML<LinesSolver2D>, u8"Side Perfectly Matched Layers boundary conditions.\n\n" PML_ATTRS_DOC);
    RO_FIELD(modes, "Computed modes.");
    solver.def("scattering", Scattering<LinesSolver2D>::from_polarization, py::with_custodian_and_ward_postcall<0, 1>(),
               (py::arg("side"), "polarization"));
    solver.def("scattering", Scattering<LinesSolver2D>::from_index, py::with_custodian_and_ward_postcall<0, 1>(),
               (py::arg("side"), "idx"));
    solver.def("scattering", Scattering<LinesSolver2D>::from_array, py::with_custodian_and_ward_postcall<0, 1>(),
               (py::arg("side"), "coeffs"),
               ...
               u8"    idx: Eigenmode number.\n"
               u8"    coeffs: expansion coefficients of the incident vector.\n\n"
               u8":rtype: Lines2D.Scattering\n");
    solver.def("layer_eigenmodes", &Eigenmodes<LinesSolver2D>::fromZ, py::arg("level"),
               ...
               u8":rtype: :class:`~optical.slab.Lines2D.Eigenmodes`\n",
               py::with_custodian_and_ward_postcall<0, 1>());
    solver.def("gaussian", &LinesSolver2D_incidentGaussian, (py::arg("side"), "polarization", "sigma", py::arg("center") = 0.),
               ...
               u8"Example:\n"
               u8"   >>> scattered = lines.scattering('top', \n"
               u8"   ...     lines.gaussian('top', 'Ex', 0.2))\n");
    solver.def("scattering_gaussian", &LinesSolver2D_scatteringGaussian,
               (py::arg("side"), "polarization", "sigma", py::arg("center") = 0.),
               u8"Helper function to Access reflected fields for access incidence.\n\n"
               u8"This method is equivalent to calling:\n\n"
               u8"   >>> lines.scattering(side,\n"
               u8"   ...     lines.gaussian(side, polarization, sigma, center))\n\n"
               u8"Args:\n"
               u8"    side (`top` or `bottom`): Side of the structure where the incident light is\n"
               u8"        present.\n"
               u8"    polarization: Specification of the incident light polarization.\n"
    ...
    register_vector_of<LinesSolver2D::Mode>("Modes");
    py::class_<LinesSolver2D::Mode>("Mode", u8"Detailed information about the mode.", py::no_init)
        .def_readonly("symmetry", &LinesSolver2D::Mode::symmetry, u8"Mode horizontal symmetry.")
        .def_readonly("polarization", &LinesSolver2D::Mode::polarization, u8"Mode polarization.")
        .add_property("lam", &getModeWavelength<LinesSolver2D::Mode>, u8"Mode wavelength [nm].")
        .add_property("wavelength", &getModeWavelength<LinesSolver2D::Mode>, u8"Mode wavelength [nm].")
        .def_readonly("k0", &LinesSolver2D::Mode::k0, u8"Mode normalized frequency [1/µm].")
        .def_readonly("beta", &LinesSolver2D::Mode::beta, u8"Mode longitudinal wavevector [1/µm].")
        .add_property("neff", &LinesSolver2D_Mode_Neff, u8"Mode longitudinal effective index [-].")
        .def_readonly("ktran", &LinesSolver2D::Mode::ktran, u8"Mode transverse wavevector [1/µm].")
        .def_readwrite("power", &LinesSolver2D::Mode::power, u8"Total power emitted into the mode [mW].")
        .def("__str__", &LinesSolver2D_Mode_str)
        .def("__repr__", &LinesSolver2D_Mode_repr)
        .def("__getattr__", &LinesSolver2D_Mode__getattr__);

    Scattering<LinesSolver2D>::registerClass("2D");
    Eigenmodes<LinesSolver2D>::registerClass("Lines2D", "2D");
}
