...
//     int info;
//     zgeev(jobvl, jobvr, int(N), A.data(), int(N), vals.data(), vl, int(N), vr, int(N), work.get(), int(lwork), rwork.get(),
//     info);

//     return info;
// }

int geneigenv(cmatrix& A, cmatrix& B, cdiagonal& vals, cmatrix* rightv, cmatrix* leftv) {
    // Check the validity of the matrices
    if (A.rows() != A.cols()) throw ComputationError("eigenv", "Matrix A should be square");
    const std::size_t N = A.rows();
    if (B.rows() != N || B.cols() != N)
        throw ComputationError("eigenv", "Matrix B should have the same number of rows and columns as the original matrix");
    if (vals.size() != N)
        throw ComputationError("eigenv", "Eigenvalues should have the same number of rows as the original matrix");
    if (rightv)
        if (rightv->rows() != N || rightv->cols() != N)
            throw ComputationError("eigenv", "Matrices for right eigenvectors should be square");
    if (leftv)
        if (leftv->rows() != N || leftv->cols() != N)
            throw ComputationError("eigenv", "Matrices for left eigenvectors should be square");
    ...
    // Create the workplace
    aligned_unique_ptr<dcomplex[]> beta(aligned_malloc<dcomplex>(N));
    const std::size_t lwork = 2 * N + 1;
    // int lwork = N*N;
    aligned_unique_ptr<dcomplex[]> work(aligned_malloc<dcomplex>(lwork));
    aligned_unique_ptr<double[]> rwork(aligned_malloc<double>(2 * N));
    ...
    int info;
    int iN = int(N);
    zggev(jobvl, jobvr, iN, A.data(), int(N), B.data(), iN, vals.data(), beta.get(), vl, iN, vr, iN, work.get(), int(lwork),
          rwork.get(), info);

    if (0 <= info && info <= N) {
        for (dcomplex *val = vals.data(), *denom = beta.get(), *end = vals.data() + N; val != end; ++val, ++denom) *val /= *denom;
    }
