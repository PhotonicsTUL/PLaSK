...
/**
 *   Compute Bessel \a J and \a Y functions for real parameter.
...
/**
 *   Compute Bessel functions of all order up to \a n using recurrence
 *   relations. If \a nm < \a n only \a nm orders are returned.
 *
 *   \param x    -- argument of Bessel function of 1st and 2nd kind.
 *   \param n    -- order
 *
 *   \param[out] nm   -- highest order actually computed (\a nm <= \a n)
 *   \param[out] jn   -- Bessel function of 1st kind, orders from 0 to \a nm
 *   \param[out] yn   -- Bessel function of 2nd kind, orders from 0 to \a nm
 *   \param[out] jnp  -- derivative of Bessel function of 1st kind, orders from 0 to \a nm
 *   \param[out] ynp  -- derivative of Bessel function of 2nd kind, orders from 0 to \a nm
 *
 *   \return error code: 0 = OK, 1 = error
 */
 ...
 *   Compute Bessel functions of all order up to \a n using recurrence
 *   relations. If \a nm < \a n only \a nm orders are returned.
 *   Different recurrence relations used for \a x < 300.
 *
 *   \param x    -- argument of Bessel function of 1st and 2nd kind.
 *   \param n    -- order
 *
...
/**
 *   The following routine computes Bessel \a Jv(\a x) and Yv(\a x) for
 *   arbitrary positive order (\a v). For negative order, use:
 *
 *   \f[
 *       J_{-v}(x) = J_{v}(x)\cos(v \pi) - Y_{v}(x)\sin(v \pi)
 *   \f]\f[
 *       Y_{-v}(x) = J_{v}(x)\sin(v \pi) + Y_{v}(x)\cos(v \pi)
 *   \f]
 *
 *   \param x    -- argument of Bessel function of 1st and 2nd kind.
 *   \param v    -- order
 *
 *   \param[out] vm   -- highest order actually computed (\a vm <= \a v)
 *   \param[out] jv   -- Bessel function of 1st kind, orders from 0 to \a vm
 *   \param[out] yn   -- Bessel function of 2nd kind, orders from 0 to \a vm
 *   \param[out] jvp  -- derivative of Bessel function of 1st kind, orders from 0 to \a vm
 *   \param[out] yvp  -- derivative of Bessel function of 2nd kind, orders from 0 to \a vm
 *
 *   \return error code: 0 = OK, 1 = error
 */
...
/**
 *   Compute Bessel \a I and \a K functions for real parameter.
 *   This algorithm computes the above functions using series expansions.
 *
...
/**
 *   Compute modified Bessel functions of all order up to \a n using recurrence
 *   relations. If \a nm < \a n only \a nm orders are returned.
 *
 *   \param x    -- argument of modified Bessel functions of 1st and 2nd kind.
 *   \param n    -- order
 *
 *   \param[out] nm   -- highest order actually computed (\a nm <= \a n)
 *   \param[out] in   -- modified Bessel function of 1st kind, orders from 0 to \a nm
 *   \param[out] kn   -- modified Bessel function of 2nd kind, orders from 0 to \a nm
 *   \param[out] inp  -- derivative of modified Bessel function of 1st kind, orders from 0 to \a nm
 *   \param[out] knp  -- derivative of modified Bessel function of 2nd kind, orders from 0 to \a nm
 *
 *   \return error code: 0 = OK, 1 = error
 */
...
/**
 *   The following routine computes modified Bessel \a Iv(\a x) and \a Kv(\a x) for
 *   arbitrary positive order (\a v).
 *
 *   \param x    -- argument of modified Bessel function of 1st and 2nd kind.
 *   \param v    -- order
 *
 *   \param[out] vm   -- highest order actually computed (\a vm <= \a v)
 *   \param[out] iv   -- modified Bessel function of 1st kind, orders from 0 to \a vm
 *   \param[out] kv   -- modified Bessel function of 2nd kind, orders from 0 to \a vm
 *   \param[out] ivp  -- derivative of modified Bessel function of 1st kind, orders from 0 to \a vm
 *   \param[out] kvp  -- derivative of modified Bessel function of 2nd kind, orders from 0 to \a vm
 */
 ...
 *
 *   \param[out] nm   -- highest order actually computed (\a nm <= \a n)
 *   \param[out] cj   -- Bessel function of 1st kind, orders from 0 to \a nm
 *   \param[out] cy   -- Bessel function of 2nd kind, orders from 0 to \a nm
 *   \param[out] cjp  -- derivative of Bessel function of 1st kind, orders from 0 to \a nm
 *   \param[out] cyp  -- derivative of Bessel function of 2nd kind, orders from 0 to \a nm
 *
 *   \return error code: 0 = OK, 1 = error
 */
 ...
 *
 *   \param[out] vm   -- highest order actually computed (\a vm <= \a v)
 *   \param[out] cj   -- Bessel function of 1st kind, orders from 0 to \a vm
 *   \param[out] cy   -- Bessel function of 2nd kind, orders from 0 to \a vm
 *   \param[out] cjvp  -- derivative of Bessel function of 1st kind, orders from 0 to \a vm
 *   \param[out] cyvp  -- derivative of Bessel function of 2nd kind, orders from 0 to \a vm
 *
 *   \return error code: 0 = OK, 1 = error
 */
 ...
 *
 *   \param[out] nm   -- highest order actually computed (\a nm <= \a n)
 *   \param[out] ci   -- modified Bessel function of 1st kind, orders from 0 to \a nm
 *   \param[out] ck   -- modified Bessel function of 2nd kind, orders from 0 to \a nm
 *   \param[out] cip  -- derivative of modified Bessel function of 1st kind, orders from 0 to \a nm
 *   \param[out] ckp  -- derivative of modified Bessel function of 2nd kind, orders from 0 to \a nm
 *
 *   \return error code: 0 = OK, 1 = error
 */
 ...
 *
 *   \param[out] vm   -- highest order actually computed (\a vm <= \a v)
 *   \param[out] civ   -- modified Bessel function of 1st kind, orders from 0 to \a vm
 *   \param[out] ckv   -- modified Bessel function of 2nd kind, orders from 0 to \a vm
 *   \param[out] civp  -- derivative of modified Bessel function of 1st kind, orders from 0 to \a vm
 *   \param[out] ckvp  -- derivative of modified Bessel function of 2nd kind, orders from 0 to \a vm
 */
