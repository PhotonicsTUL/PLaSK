...
void SimpleOptical::loadConfiguration(XMLReader& reader, Manager& manager) {
    //MD: zupełnie brakuje ładowania konfiguracji (ustawienie rootdiggera i współrzędnej poziomej `x`, w której szukane jest rozwiązanie)
...
//MD: Proszę ustawić wcięcia na 4 spacje i pisać nawiasy { na końcu linijki (z ew. wyjątkiem metod i funkcji) — dotyczy to całości kodu (więcej nie będę o tym pisał)
...
size_t SimpleOptical::findMode(double lambda, int m)
{
    ...
    onInitialize();
    Data2DLog<dcomplex,dcomplex> log_stripe(getId(), format(""), "", ""); //MD: format tutaj nie ma sensu; proszę podać sensowne wartości do wyświetlenia
    auto rootdigger = RootDigger::get(this,
    //MD: lepiej robić wcięcia spacjami
				      [&](const dcomplex& x ){
...
dcomplex SimpleOptical::computeTransferMatrix(const dcomplex& x, const std::vector<dcomplex> & NR)
{
  ...
  setWavelength(w);
  onInitialize(); //MD: powinno być `initCalculation` — `onInitializa` zostanie wywołane samo tylko wtedy gdy to będzie potrzebne
  initializeRefractiveIndexVec(); //MD: to powinno być wywołane tylko wtedy gdy nie ma skeszowanych współczynników załamania bądź są one nieaktualne (np. zmieniła się długość fali)
  //MD: Swoją drogą może warto dodać parametr konfiguracyjny pozwalający użytkownikowi zdecydować czy chce pobierać współczynniki załamania dla każdej długości fali osobno czy nie
  Matrix phas_matrix;
...
dcomplex SimpleOptical::getVertDeterminant(double wavelength)
{
  ...
  initializeRefractiveIndexVec(); //MD: to już jest w `computeTransferMatrix`
...
{
  onInvalidate(); //MD: nie powinno tego być
  setWavelength(real(modes[num].lam)); //MD: nie sprawdza Pan czy mod o numerze `num` został znaleziony (istnieje w tablicy)
  onInitialize();  //MD: powinno być `initCalculation` — `onInitialize` zostanie wywołane samo tylko wtedy gdy to będzie potrzebne
  vecE.clear();
