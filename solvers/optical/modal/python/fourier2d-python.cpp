...
template <> inline const char* solver_compute_reflectivity_name<FourierSolver2D>() {
    return "Fourier2D.compute_reflectivity";
}

template <> inline const char* solver_compute_transmittivity_name<FourierSolver2D>() {
    return "Fourier2D.compute_transmittivity";
}


template <> py::object Eigenmodes<FourierSolver2D>::array(const dcomplex* data, size_t N) const {
    int dim = 2, strid = 2;
...
static dcomplex FourierSolver2D_Mode_Neff(const FourierSolver2D::Mode& mode) { return mode.beta / mode.k0; }
...
static shared_ptr<Scattering<FourierSolver2D>> FourierSolver2D_scatteringGaussian(FourierSolver2D& self,
                                                                                  Transfer::IncidentDirection side,
                                                                                  Expansion::Component polarization,
                                                                                  double sigma,
                                                                                  double center) {
    return shared_ptr<Scattering<FourierSolver2D>>(
...
void export_FourierSolver2D() {
    ...
    CLASS(FourierSolver2D, "Fourier2D",
          ...
          u8"It calculates optical modes and optical field distribution using Fourier modal method\n"
    ...
    solver.def("scattering", Scattering<FourierSolver2D>::from_index, py::with_custodian_and_ward_postcall<0, 1>(),
               (py::arg("side"), "idx"));
    solver.def("scattering", Scattering<FourierSolver2D>::from_array, py::with_custodian_and_ward_postcall<0, 1>(),
               (py::arg("side"), "coeffs"),
    ...
    solver.def("layer_eigenmodes", &Eigenmodes<FourierSolver2D>::fromZ, py::arg("level"),
               ...
               u8"Args:\n"
               u8"    level (float): Vertical level at which the coefficients are computed.\n\n"
               u8":rtype: :class:`~optical.modal.Fourier2D.Eigenmodes`\n",
    ...
    solver.def("scattering_gaussian", &FourierSolver2D_scatteringGaussian,
