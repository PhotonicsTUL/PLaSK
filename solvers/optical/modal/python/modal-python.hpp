...
template <typename SolverT>
static py::tuple ModalSolver_getStack(SolverT& self) {
...
template <typename SolverT>
static shared_ptr<OrderedAxis> ModalSolver_getLayerEdges(SolverT& self) {
    self.Solver::initCalculation();
    return make_shared<OrderedAxis>(*self.vbounds);
}

template <typename SolverT>
static shared_ptr<OrderedAxis> ModalSolver_getLayerCenters(SolverT& self) {
    self.Solver::initCalculation();
    return make_shared<OrderedAxis>(*self.verts);
}

// template <typename SolverT>
// static py::tuple ModalSolver_getLayerSets(const SolverT& self) {
//     py::list result;
    ...
    static void registerClass(const char* solver, const char* suffix) {
        ...
        py::class_<Eigenmode>("Eigenmode",
            format("Single eignemode. You can access object of this class by indexing\n"
                   ":class:`~optical.modal.{}.Eigenmodes` object.", solver).c_str(),
            py::no_init)
...
struct Scattering {
    ...
    struct Reflected {
        ...
        Reflected(Scattering* parent):
            parent(parent),
            outLightE(this, &Scattering::Reflected::getLightE),
            outLightH(this, &Scattering::Reflected::getLightH) {}
            ...
            py::class_<Scattering<SolverT>::Reflected, boost::noncopyable>("Reflected", "Reflected field details", py::no_init)
                ...
                .add_property("eigenmodes", py::make_function(&Scattering<SolverT>::Reflected::eigenmodes, py::with_custodian_and_ward_postcall<0,1>()),
                            format("Reflected eigenmodes.\n\n"
                                    ":rtype: :class:`~optical.modal.{}{}.Eigenmodes`", name, suffix).c_str()
                            )
    ...
    struct Incident {
        ...
        Incident(Scattering* parent):
            parent(parent),
            outLightE(this, &Scattering::Incident::getLightE),
            outLightH(this, &Scattering::Incident::getLightH) {}
        py::object get_coefficients() {
    ...
    struct Transmitted {
        ...
        Transmitted(Scattering* parent):
            parent(parent),
            outLightE(this, &Scattering::Transmitted::getLightE),
            outLightH(this, &Scattering::Transmitted::getLightH) {}
        py::object get_coefficients() {
            if (!parent->solver->initCalculation()) parent->solver->setExpansionDefaults();
...
inline void export_base(Class solver) {
    ...
    solver.add_property("stack", &ModalSolver_getStack<Solver>, "Stack of distinct layers.");
    solver.add_property("layer_edges", &ModalSolver_getLayerEdges<Solver>, "Vertical posiotions of egges of each layer.");
    solver.add_property("layer_centers", &ModalSolver_getLayerCenters<Solver>,
                        "Vertical posiotions of centers of each layer.\n\n"
                        "At these positions materials and temperatures are probed.\n");
    // solver.add_property("layer_sets", &ModalSolver_getLayerSets<Solver>, "Vertical positions of layers in each layer set.");
    ...
    solver.add_provider("outUpwardsLightE", &Solver::outUpwardsLightE, "");
    solver.add_provider("outUpwardsLightH", &Solver::outUpwardsLightH, "");
    solver.add_provider("outDownwardsLightE", &Solver::outDownwardsLightE, "");
    solver.add_provider("outDownwardsLightH", &Solver::outDownwardsLightH, "");
    ...
    solver.def_readwrite("determinant_type", &ModalBase::determinant_type,
