...
ReflectionTransfer::ReflectionTransfer(ModalBase* solver, Expansion& expansion, Matching matching): Transfer(solver, expansion),
    matching(matching),
    storeP(STORE_NONE) {
...
void ReflectionTransfer::determineFields()
{
    ...
    if (solver->emission == ModalBase::EMISSION_BOTTOM || solver->emission == ModalBase::EMISSION_TOP) {
        size_t n = (solver->emission == ModalBase::EMISSION_BOTTOM)? 0 : count-1;
