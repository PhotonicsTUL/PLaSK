...
void ModalBase::initTransfer(Expansion& expansion, bool reflection) {
...
ModalSolver<BaseT>::ModalSolver(const std::string& name)
    : BaseT(name),
      smooth(0.),
      outEpsilon(this, &ModalSolver<BaseT>::getEpsilonProfile),
      outRefractiveIndex(this, &ModalSolver<BaseT>::getRefractiveIndex),
      outWavelength(this, &ModalSolver<BaseT>::getWavelength, &ModalSolver<BaseT>::nummodes),
      outLightMagnitude(this, &ModalSolver<BaseT>::getLightMagnitude, &ModalSolver<BaseT>::nummodes),
      outLightE(this, &ModalSolver<BaseT>::getLightE<>, &ModalSolver<BaseT>::nummodes),
      outLightH(this, &ModalSolver<BaseT>::getLightH<>, &ModalSolver<BaseT>::nummodes),
      outUpwardsLightE(this, &ModalSolver<BaseT>::getLightE<PROPAGATION_UPWARDS>, &ModalSolver<BaseT>::nummodes),
      outUpwardsLightH(this, &ModalSolver<BaseT>::getLightH<PROPAGATION_UPWARDS>, &ModalSolver<BaseT>::nummodes),
      outDownwardsLightE(this, &ModalSolver<BaseT>::getLightE<PROPAGATION_DOWNWARDS>, &ModalSolver<BaseT>::nummodes),
      outDownwardsLightH(this, &ModalSolver<BaseT>::getLightH<PROPAGATION_DOWNWARDS>, &ModalSolver<BaseT>::nummodes) {
    inTemperature = 300.;  // temperature receiver has some sensible value
    this->inTemperature.changedConnectMethod(this, &ModalSolver<BaseT>::onInputChanged);
    this->inGain.changedConnectMethod(this, &ModalSolver<BaseT>::onGainChanged);
    this->inCarriersConcentration.changedConnectMethod(this, &ModalSolver<BaseT>::onInputChanged);
}

template <typename BaseT> ModalSolver<BaseT>::~ModalSolver() {
    this->inTemperature.changedDisconnectMethod(this, &ModalSolver<BaseT>::onInputChanged);
    this->inGain.changedDisconnectMethod(this, &ModalSolver<BaseT>::onGainChanged);
    this->inCarriersConcentration.changedDisconnectMethod(this, &ModalSolver<BaseT>::onInputChanged);
}

std::unique_ptr<RootDigger> ModalBase::getRootDigger(const RootDigger::function_type& func, const char* name) {
...
template <typename BaseT> void ModalSolver<BaseT>::parseCommonModalConfiguration(XMLReader& reader, Manager& manager) {
...
template <typename BaseT> void ModalSolver<BaseT>::setupLayers() {
...
template <typename BaseT>
DataVector<const Tensor3<dcomplex>> ModalSolver<BaseT>::getEpsilonProfile(
...
template <typename BaseT>
LazyData<dcomplex> ModalSolver<BaseT>::getRefractiveIndex(RefractiveIndex::EnumType component,
                                                         const shared_ptr<const MeshD<BaseT::SpaceType::DIM>>& dst_mesh,
                                                         dcomplex lam,
                                                         InterpolationMethod interp) {
...
#ifndef NDEBUG
void ModalBase::getMatrices(size_t layer, cmatrix& RE, cmatrix& RH) {
    initCalculation();
    computeIntegrals();
...
template <typename BaseT> size_t ModalSolver<BaseT>::initIncidence(Transfer::IncidentDirection side, dcomplex lam) {
    Expansion& expansion = getExpansion();
...
template <typename BaseT> cvector ModalSolver<BaseT>::incidentVector(Transfer::IncidentDirection side, size_t idx, dcomplex lam) {
...
template <typename BaseT>
cvector ModalSolver<BaseT>::incidentVector(Transfer::IncidentDirection side, const cvector& incident, dcomplex lam) {
    size_t layer = initIncidence(side, lam);
...
void ModalBase::scaleIncidentVector(cvector& incident, size_t layer, double size_factor) {
    double norm2 = 0.;
...
template <> void ModalSolver<SolverWithMesh<Geometry2DCartesian, MeshAxis>>::scaleIncidentVector(cvector& incident, size_t layer) {
    ModalBase::scaleIncidentVector(incident, layer, 1e-3);  // sqrt(µm -> m)
}

template <> void ModalSolver<SolverWithMesh<Geometry2DCylindrical, MeshAxis>>::scaleIncidentVector(cvector& incident, size_t layer) {
    throw NotImplemented(getId(), "CylindicalSolver::incidentVector");
}

template <> void ModalSolver<SolverOver<Geometry3D>>::scaleIncidentVector(cvector& incident, size_t layer) {
    ModalBase::scaleIncidentVector(incident, layer, 1e-6);  // sqrt(µm² -> m²)
}

dvector ModalBase::getIncidentFluxes(const cvector& incident, Transfer::IncidentDirection side) {
    initCalculation();
...
dvector ModalBase::getReflectedFluxes(const cvector& incident, Transfer::IncidentDirection side) {
...
dvector ModalBase::getTransmittedFluxes(const cvector& incident, Transfer::IncidentDirection side) {
...
cvector ModalBase::getReflectedCoefficients(const cvector& incident, Transfer::IncidentDirection side) {
    initCalculation();
    if (!transfer) initTransfer(getExpansion(), true);
...
cvector ModalBase::getTransmittedCoefficients(const cvector& incident, Transfer::IncidentDirection side) {
    initCalculation();
    if (!transfer) initTransfer(getExpansion(), true);
...
LazyData<Vec<3, dcomplex>> ModalSolver<BaseT>::getLightE(size_t num,
...
template <typename BaseT>
template <PropagationDirection part>
LazyData<Vec<3, dcomplex>> ModalSolver<BaseT>::getLightH(size_t num,
                                                        shared_ptr<const MeshD<BaseT::SpaceType::DIM>> dst_mesh,
                                                        InterpolationMethod method) {
    assert(transfer);
    double power = applyMode(num);
...
template <typename BaseT>
LazyData<double> ModalSolver<BaseT>::getLightMagnitude(size_t num,
                                                      shared_ptr<const MeshD<BaseT::SpaceType::DIM>> dst_mesh,
                                                      InterpolationMethod method) {
    assert(transfer);
    double power = applyMode(num);
...
// template class PLASK_SOLVER_API ModalSolver<SolverOver<Geometry2DCartesian>>;
