...
struct PLASK_SOLVER_API DriftDiffusionModel2DSolver : public FemSolverWithMesh<Geometry2DType, RectangularMesh<2>> {
  ...
  public:
    ...
    */
    ...
    double maxerrPsiI;  ///< Maximum estimated error for initial potential during all iterations (useful for single calculations
                        ///< managed by external python script)
    double maxerrPsi0;  ///< Maximum estimated error for potential at U = 0 V during all iterations (useful for single calculations
                        ///< managed by external python script)
    double maxerrPsi;   ///< Maximum estimated error for potential during all iterations (useful for single calculations managed by
                        ///< external python script)
    double maxerrFn;    ///< Maximum estimated error for quasi-Fermi energy level for electrons during all iterations (useful for
                        ///< single calculations managed by external python script)
    double maxerrFp;    ///< Maximum estimated error for quasi-Fermi energy level for holes during all iterations (useful for single
                        ///< calculations managed by external python script)
     ...
     * \return true when carriers are confined
     */
