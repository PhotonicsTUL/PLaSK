...
template <typename Geometry2DType> void Capacitance2DSolver<Geometry2DType>::compute() {
    ...
    // Compute active current densities
    for (auto el : this->maskedMesh->elements()) {
        ...
        currents[i] = vec(real(conds[i].c00) * dvx, real(conds[i].c11) * dvy);
    }
...
template <typename Geometry2DType> dcomplex Capacitance2DSolver<Geometry2DType>::getS11() {
    return 0;  // TODO
}
