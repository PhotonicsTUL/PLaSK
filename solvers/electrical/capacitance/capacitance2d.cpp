...
void Capacitance2DSolver<Geometry2DType>::parseConfiguration(XMLReader& source, Manager& manager) {
    ...
    if (param == "ac-voltage") {
        ...
        frequency = source.getAttribute<double>("frequency", frequency);
...
void Capacitance2DSolver<Geometry2DType>::setMatrix(
    ...
    for (auto e : this->maskedMesh->elements()) {
        ...
        dcomplex kx = this->conductivities[i].c00;
        dcomplex ky = this->conductivities[i].c11;
...
template <typename Geometry2DType> void Capacitance2DSolver<Geometry2DType>::loadConductivities() {
    ...
    conductivities.reset(midmesh->size());  ///< Cached complex conductivities

    for (auto e : this->maskedMesh->elements()) {
        size_t i = e.getIndex();
        Vec<2, double> midpoint = e.getMidpoint();
        double imag =
            2e9 * M_PI * phys::epsilon0 * frequency * this->geometry->getMaterial(midpoint)->eps(temperature[i]);  // GHz -> Hz
        conductivities[i] = Tensor2<dcomplex>(dcomplex(conductivity[i].c00, imag), dcomplex(conductivity[i].c11, imag));
    }
}
...
template <typename Geometry2DType> void Capacitance2DSolver<Geometry2DType>::compute() {
    ...
    this->writelog(LOG_INFO, "Running AC calculations for {} GHz", frequency);
...
template <typename Geometry2DType>
Vec<2, dcomplex> Capacitance2DSolver<Geometry2DType>::getElementCurrentDensity(const RectangularMaskedMesh2D::Element& element,
                                                                               bool active_current) const {
    ...
    dcomplex dvx = -0.05 * (-potentials[loleftno] + potentials[lorghtno] - potentials[upleftno] + potentials[uprghtno]) /
                   (element.getUpper0() - element.getLower0());  // [j] = kA/cm²
    dcomplex dvy = -0.05 * (-potentials[loleftno] - potentials[lorghtno] + potentials[upleftno] + potentials[uprghtno]) /
                   (element.getUpper1() - element.getLower1());  // [j] = kA/cm²

    if (!active_current)
        return vec(conductivities[index].c00 * dvx, conductivities[index].c11 * dvy);
    else
        return vec(conductivities[index].c00.real() * dvx, conductivities[index].c11.real() * dvy);
}

template <typename Geometry2DType> void Capacitance2DSolver<Geometry2DType>::computeCurrentDensities() {
    this->writelog(LOG_DEBUG, "Computing current densities");
    currents.reset(this->maskedMesh->getElementsCount(), vec(0., 0.));
    for (auto element : this->maskedMesh->elements()) {
        currents[element.getIndex()] = getElementCurrentDensity(element);
    }
}

template <>
dcomplex Capacitance2DSolver<Geometry2DCartesian>::integrateCurrent(size_t vindex, bool active_current, bool onlyactive) const {
    ...
    for (size_t i = 0; i < mesh->axis[0]->size() - 1; ++i) {
        ...
        if (!onlyactive || isActive(element.getMidpoint())) {
            if (element.getIndex() != RectangularMaskedMesh2D::Element::UNKNOWN_ELEMENT_INDEX) {
                result += getElementCurrentDensity(element, active_current).c1 * element.getSize0();
            }
        }
    }
...
template <>
dcomplex Capacitance2DSolver<Geometry2DCylindrical>::integrateCurrent(size_t vindex, bool active_current, bool onlyactive) const {
    if (!potentials) throw NoValue("current densities");
    this->writelog(LOG_DETAIL, "Computing total current");
    dcomplex result = 0.;
    for (size_t i = 0; i < mesh->axis[0]->size() - 1; ++i) {
        ...
        if (!onlyactive || isActive(element.getMidpoint())) {
            ...
            if (index != RectangularMaskedMesh2D::Element::UNKNOWN_ELEMENT_INDEX) {
                ...
                result += getElementCurrentDensity(element, active_current).c1 * (rout * rout - rin * rin);
            }
        }
    }
...
template <typename Geometry2DType>
dcomplex Capacitance2DSolver<Geometry2DType>::getAcCurrent(size_t nact, bool active_current) const {
    if (!potentials) throw NoValue("current");
    if (nact >= this->active.size()) throw BadInput(this->getId(), "wrong active region number");
    const auto& act = this->active[nact];
    // Find the average of the active region
    size_t level = (act.bottom + act.top) / 2;
    return integrateCurrent(level, active_current, true);
}

template <typename Geometry2DType> dcomplex Capacitance2DSolver<Geometry2DType>::getImpedance() const {
    if (!potentials) throw NoValue("impedance");
    if (this->voltage_boundary.size() != 2)
        throw Exception("{0}: Impedance can be computed only for 2-terminal structures", this->getId());
    dcomplex voltage = this->voltage_boundary[0].value - this->voltage_boundary[1].value;
    dcomplex current = 1e-3 * getAcCurrent();  // Convert from mA to A
    dcomplex impedance = voltage / current;
    if (impedance.real() < 0.) impedance = -impedance;  // make sure that the real part is positive
    return impedance;
}
...
const LazyData<Vec<2, dcomplex>> Capacitance2DSolver<Geometry2DType>::getCurrentDensities(shared_ptr<const MeshD<2>> dest_mesh,
    ...
    if (!currents) computeCurrentDensities();
    if (method == INTERPOLATION_DEFAULT) method = INTERPOLATION_LINEAR;
...
template <> std::string Capacitance2DSolver<Geometry2DCartesian>::getClassName() const { return "Capacitance2D"; }
template <> std::string Capacitance2DSolver<Geometry2DCylindrical>::getClassName() const { return "CapacitanceCyl"; }
