...
ElectricalFem2DSolver<GeometryT>::ElectricalFem2DSolver(const std::string& name)
    : SolverWithMesh<GeometryT, RectangularMesh<2>>(name),
      pcond(5.),
      ncond(50.),
      loopno(0),
      ...
      use_full_mesh(false),
      maxerr(0.05),
      outVoltage(this, &ElectricalFem2DSolver<GeometryT>::getVoltage),
      outCurrentDensity(this, &ElectricalFem2DSolver<GeometryT>::getCurrentDensities),
      outHeat(this, &ElectricalFem2DSolver<GeometryT>::getHeatDensities),
      outConductivity(this, &ElectricalFem2DSolver<GeometryT>::getConductivity),
...
template <typename GeometryT>
void ElectricalFem2DSolver<GeometryT>::parseConfiguration(XMLReader& source, Manager& manager) {
...
template <typename GeometryT> ElectricalFem2DSolver<GeometryT>::~ElectricalFem2DSolver() {}

template <typename GeometryT> void ElectricalFem2DSolver<GeometryT>::setActiveRegions() {
    this->invalidate();
...
template <typename GeometryT> void ElectricalFem2DSolver<GeometryT>::onInitialize() {
...
template <typename GeometryT> void ElectricalFem2DSolver<GeometryT>::onInvalidate() {
    conds.reset();
    potentials.reset();
    currents.reset();
    heats.reset();
...
void ElectricalFem2DSolver<GeometryT>::applyBC(
    MatrixT& A,
    DataVector<double>& B,
...
template <typename GeometryT>
template <typename MatrixT>
void ElectricalFem2DSolver<GeometryT>::setMatrix(
    MatrixT& A,
    DataVector<double>& B,
    const BoundaryConditionsWithMesh<RectangularMesh<2>::Boundary, double>& bvoltage,
...
template <typename GeometryT> template <typename MatrixT> MatrixT ElectricalFem2DSolver<GeometryT>::makeMatrix() {
    if (band == 0) {
        if (use_full_mesh) {
...
template <typename GeometryT>
template <typename MatrixT>
double ElectricalFem2DSolver<GeometryT>::doCompute(unsigned loops) {
...
template <typename GeometryT> void ElectricalFem2DSolver<GeometryT>::solveMatrix(DpbMatrix& A, DataVector<double>& B) {
    int info = 0;
...
template <typename GeometryT> void ElectricalFem2DSolver<GeometryT>::solveMatrix(DgbMatrix& A, DataVector<double>& B) {
    int info = 0;
    this->writelog(LOG_DETAIL, "Solving matrix system");
...
template <typename GeometryT>
void ElectricalFem2DSolver<GeometryT>::solveMatrix(SparseBandMatrix2D& A, DataVector<double>& B) {
    this->writelog(LOG_DETAIL, "Solving matrix system");
...
template <typename GeometryT> void ElectricalFem2DSolver<GeometryT>::saveHeatDensities() {
...
template <typename GeometryT> double ElectricalFem2DSolver<GeometryT>::getTotalCurrent(size_t nact) {
...
template <typename GeometryT>
const LazyData<double> ElectricalFem2DSolver<GeometryT>::getVoltage(shared_ptr<const MeshD<2>> dest_mesh,
...
template <typename GeometryT>
const LazyData<Vec<2>> ElectricalFem2DSolver<GeometryT>::getCurrentDensities(shared_ptr<const MeshD<2>> dest_mesh,
...
template <typename GeometryT>
const LazyData<double> ElectricalFem2DSolver<GeometryT>::getHeatDensities(shared_ptr<const MeshD<2>> dest_mesh,
                                                                               InterpolationMethod method) {
...
template <typename GeometryT>
const LazyData<Tensor2<double>> ElectricalFem2DSolver<GeometryT>::getConductivity(shared_ptr<const MeshD<2>> dest_mesh,
                                                                                       InterpolationMethod) {
    this->initCalculation();
...
template <typename GeometryT> double ElectricalFem2DSolver<GeometryT>::getCapacitance() {
