...
class SVD 
{
      ...
      while (p > 0) {
         ...
         if (k == p-2) {
            ...
            for (ks = p-1; ks >= k; ks--) {
               ...
               long double t = (ks != p ? fabs(e[ks]) : 0.) + 
         ...
         switch (kase) {
            ...
            case 1: {
               ...
               e[p-2] = 0.0;
               for (j = p-2; j >= k; j--) {
                  long double t = hypot(s[j],f);
                  long double cs = s[j]/t;
                  long double sn = f/t;
                  s[j] = t;
                  if (j != k) {
                     f = -sn*e[j-1];
            ...
            case 3: {
               ...
               long double scale = std::max(std::max(std::max(std::max(
                       fabs(s[p-1]),fabs(s[p-2])),fabs(e[p-2])), 
                       fabs(s[k])),fabs(e[k]));
               long double sp = s[p-1]/scale;
               long double spm1 = s[p-2]/scale;
               long double epm1 = e[p-2]/scale;
               long double sk = s[k]/scale;
   ...
   */
   ...
   long double cond () {
      return s[0]/s[minInt(m,n)-1]; // LUKASZ
   }
   ...
   */
   ...
   {
      long double eps = pow(2.0,-52.0);
      long double tol = maxInt(m,n)*s[0]*eps; // LUKASZ
      int r = 0;
