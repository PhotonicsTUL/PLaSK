...
namespace plask { namespace solvers { namespace fermi_golden {

template <typename GeometryType>
FermiGoldenGainSolver<GeometryType>::FermiGoldenGainSolver(const std::string& name): SolverWithMesh<GeometryType,OrderedMesh1D>(name),
    outGain(this, &FermiGoldenGainSolver<GeometryType>::getGain),
    outLuminescence(this, &FermiGoldenGainSolver<GeometryType>::getLuminescence),
    outGainOverCarriersConcentration(this, &FermiGoldenGainSolver<GeometryType>::getDgDn)
{
...
template <typename GeometryType>
void FermiGoldenGainSolver<GeometryType>::onInitialize() // In this function check if geometry and mesh are set
{
...
template <typename GeometryType>
void FermiGoldenGainSolver<GeometryType>::onInvalidate() // This will be called when e.g. geometry or mesh changes and your results become outdated
{
    region_levels.clear();
}
...
template <typename GeometryType>
void FermiGoldenGainSolver<GeometryType>::
    findEnergyLevels(typename FermiGoldenGainSolver<GeometryType>::Levels& levels, const ActiveRegionInfo& region, double iT, bool iShowSpecLogs)
{
...
template <typename GeometryType>
int FermiGoldenGainSolver<GeometryType>::buildStructure(Levels& levels, double T, const ActiveRegionInfo& region, bool iShowSpecLogs)
{
...
template <typename GeometryType>
int FermiGoldenGainSolver<GeometryType>::buildEc(Levels& levels, double T, const ActiveRegionInfo& region, bool iShowSpecLogs)
{
    levels.mpEc.clear();
...
template <typename GeometryType>
int FermiGoldenGainSolver<GeometryType>::buildEvhh(Levels& levels, double T, const ActiveRegionInfo& region, bool iShowSpecLogs)
{
    levels.mpEvhh.clear();
...
template <typename GeometryType>
int FermiGoldenGainSolver<GeometryType>::buildEvlh(Levels& levels, double T, const ActiveRegionInfo& region, bool iShowSpecLogs)
{
    levels.mpEvlh.clear();
...
template <typename GeometryType>
QW::Gain FermiGoldenGainSolver<GeometryType>::getGainModule(double wavelength, double T, double n,
                                                         const ActiveRegionInfo& region, const Levels& levels, bool iShowSpecLogs)
{
    ...
    if ((!levels.mEc)&&((!levels.mEvhh)||(!levels.mEvlh)))
    {
        ...
        //this->writelog(LOG_DEBUG, "mEgCladT z FermiGolden: %1% eV", tEgClad);
...
template <typename GeometryT>
struct FermiGoldenGainSolver<GeometryT>::LuminescenceData: public FermiGoldenGainSolver<GeometryT>::DataBase
{
    template <typename... Args>
...
template <typename GeometryType>
const LazyData<double> FermiGoldenGainSolver<GeometryType>::getGain(const shared_ptr<const MeshD<2>>& dst_mesh, double wavelength, InterpolationMethod interp)
{
...
template <typename GeometryType>
const LazyData<double> FermiGoldenGainSolver<GeometryType>::getDgDn(const shared_ptr<const MeshD<2>>& dst_mesh, double wavelength, InterpolationMethod interp)
{
...
template <typename GeometryType>
const LazyData<double> FermiGoldenGainSolver<GeometryType>::getLuminescence(const shared_ptr<const MeshD<2>>& dst_mesh, double wavelength, InterpolationMethod interp)
{
...
template <typename GeometryType>
LuminescenceSpectrum<GeometryType> FermiGoldenGainSolver<GeometryType>::getLuminescenceSpectrum(const Vec<2>& point)
{
    this->initCalculation();
    return LuminescenceSpectrum<GeometryType>(this, point);
}

template <> std::string FermiGoldenGainSolver<Geometry2DCartesian>::getClassName() const { return "gain.FermiGolden2D"; }
template <> std::string FermiGoldenGainSolver<Geometry2DCylindrical>::getClassName() const { return "gain.FermiGoldenCyl"; }
