#ifndef PLASK__SOLVER_GAIN_FERMISOLVER_H
#define PLASK__SOLVER_GAIN_FERMISOLVER_H
...
struct PLASK_SOLVER_API FermiGoldenGainSolver: public SolverWithMesh<GeometryType,OrderedMesh1D>
{
...
struct GainSpectrum {
    ...
    typename FermiGoldenGainSolver<GeometryT>::Levels levels; ///< Computed energy levels
    QW::Gain gMod;
    bool gModExist;

    GainSpectrum(FermiGoldenGainSolver<GeometryT>* solver, const Vec<2> point):
...
struct LuminescenceSpectrum {
    ...
    LuminescenceSpectrum(FermiGoldenGainSolver<GeometryT>* solver, const Vec<2> point):
        solver(solver), point(point), gModExist(false)
    {
        auto mesh = make_shared<const OnePointMesh<2>>(point);
        T = solver->inTemperature(mesh)[0];
...
#endif // PLASK__SOLVER_GAIN_FERMISOLVER_H
