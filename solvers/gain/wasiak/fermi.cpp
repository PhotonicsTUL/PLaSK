...
FermiGainSolver<GeometryType>::FermiGainSolver(const std::string& name): SolverWithMesh<GeometryType, MeshAxis>(name),
...
struct FermiGainSolver<GeometryT>::DataBase: public LazyDataImpl<Tensor2<double>>
{
    struct AveragedData {
...
template <typename GeometryType>
const LazyData<Tensor2<double>> FermiGainSolver<GeometryType>::getGain(Gain::EnumType what, const shared_ptr<const MeshD<2>>& dst_mesh, double wavelength, InterpolationMethod interp)
{
    if (what == Gain::DGDN) {
