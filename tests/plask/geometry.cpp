...
struct Leafs2d {
    std::shared_ptr<plask::Material> dumpMaterial;
    plask::Box<2> box_5_3;
    Leafs2d(): dumpMaterial(new DumpMaterial()), box_5_3(plask::vec(5.0, 3.0), dumpMaterial) {}
};

BOOST_AUTO_TEST_SUITE(geometry) // MUST be the same as the file name

    BOOST_FIXTURE_TEST_CASE(leaf_box2d, Leafs2d) {
        BOOST_CHECK_EQUAL(box_5_3.getBoundingBox().upper, plask::vec(5.0, 3.0));
        BOOST_CHECK_EQUAL(box_5_3.getBoundingBox().lower, plask::vec(0.0, 0.0));
        BOOST_CHECK_EQUAL(box_5_3.getMaterial(plask::vec(4.0, 2.0)), dumpMaterial);
        BOOST_CHECK(box_5_3.getMaterial(plask::vec(6.0, 2.0)) == nullptr);
        BOOST_CHECK_NO_THROW(box_5_3.validate());
    }

    BOOST_FIXTURE_TEST_CASE(translate2d, Leafs2d) {
        plask::Translation<2> translation(box_5_3, plask::vec(10.0, 20.0));    //should be in [10, 20] - [15, 23]
