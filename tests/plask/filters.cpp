...
BOOST_AUTO_TEST_SUITE(filters) // MUST be the same as the file name
    ...
    BOOST_AUTO_TEST_CASE(cartesian2D) {
        ...
        plask::Filter<DoubleField, plask::Geometry2DCartesian> filter2D(plask::make_shared<plask::Geometry2DCartesian>(container, 10.0));

        filter2D.setDefault(2.0);
        filter2D.appendInner(block11) = 1.0;
        BOOST_CHECK_EQUAL(filter2D.out(plask::toMesh(plask::vec(0.5, 0.5)), plask::DEFAULT_INTERPOLATION), plask::DataVector<double>{ 2.0 });
        BOOST_CHECK_EQUAL(filter2D.out(plask::toMesh(plask::vec(1.5, 1.5)), plask::DEFAULT_INTERPOLATION), plask::DataVector<double>{ 1.0 });
