...
BOOST_AUTO_TEST_SUITE(interpolation) // MUST be the same as the file name
    ...
    BOOST_AUTO_TEST_CASE(interpolation_choice) {
        ...
        shared_ptr<std::vector<int>> src_data {new std::vector<int> {1, 2}};
        ...
        shared_ptr<const std::vector<int>>ret_data = plask::interpolate<plask::DummyMesh,int>(src_mesh, src_data, dst_mesh, plask::LINEAR);
